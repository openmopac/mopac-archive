...
c Common MOLKST splitted in MOLKSI and MOLKSR    Ivan Rossi 0394   &8)
     ...
     2                NELECS,NALPHA,NBETA,NCLOSE,NOPEN
      ...
      CHARACTER*2 ELEMNT
      DIMENSION VANRAD(120), IJF(10), FIJ(10)
CSAV         SAVE                                                           GL0892
      DATA VANRAD/
     1   0.32,0.93,
...
C
      IF (DOPRNT) WRITE(6,'(''1'',//10X,''DESCRIPTION OF VIBRATIONS'',/)CIO
     &                  ')                                              CIO
      ILINE=0
      DO 120 K=1,N3
...
C#      WRITE(6,*)FIJ(I),IJF(I)
  ...
  100    LINEAR=I
     ...
     1                  /(SHIFT*RIJ(IJF(IJ))))**2*100.D0
               IF (VIB1) THEN
               IF (DOPRNT) WRITE(6,'(/,'' VIBRATION'',I4,               CIO
     &                           ''            ATOM PAIR '',            CIO
     &              ''    ENERGY CONTRIBUTION              RADIAL'')')K CIO
                  ANS=100.D0*SQRT(FIJ(IJ)*1.D5*6.023D23)/(2.998D10*3.141
     159D0*2.D0)/EIGS(K)
                  ANS=MIN(999.9D0,MAX(-99.9D0,ANS))
                  IF (DOPRNT) WRITE(6,'('' FREQ.   '',F9.2,6X,1A2,I2,   CIO
     1'' -- '',A2,I2, ''         '',F6.1,''% ('',F5.1,''%)'',F18.1      CIO
     2,''%'')')                                                         CIO
     3EIGS(K),ELEMNT(NAT(I)),I,ELEMNT(NAT(J)),J,E,ANS,RADIAL            CIO
C#      WRITE(6,*)ANS
                  VIB1=.FALSE.
               ELSEIF (VIB2) THEN
                  VIB2=.FALSE.
                  IF (DOPRNT) WRITE(6,'('' T-DIPOLE'',F9.4,6X,1A2,I2,   CIO
     1'' -- '',A2,I2, ''         '',F6.1,''%'',F27.1,''%'')')           CIO
     2DIPT(K),ELEMNT(NAT(I)),I,ELEMNT(NAT(J)),J,E,RADIAL                CIO
               ELSEIF (VIB3) THEN
                  VIB3=.FALSE.
                  IF (DOPRNT) WRITE(6,'('' TRAVEL  '',F9.4,6X,1A2,I2,   CIO
     1'' -- '',A2,I2, ''         '',F6.1,''%'',F27.1,''%'')')           CIO
     2TRAVEL(K),ELEMNT(NAT(I)),I,ELEMNT(NAT(J)),J,E,RADIAL              CIO
               ELSEIF (VIB4) THEN
                  VIB4=.FALSE.
                  IF (DOPRNT) WRITE(6,'('' RED. MASS'',F8.4,6X,1A2,I2,  CIO
     1'' -- '',A2,I2, ''         '',F6.1,''%'',F27.1,''%'')')           CIO
     2REDMAS(K),ELEMNT(NAT(I)),I,ELEMNT(NAT(J)),J,E,RADIAL              CIO
             ELSE
              ILINE=ILINE+1
              IF (DOPRNT) WRITE(6,'(''                        '',1A2,I2,CIO
     1'' -- '',A2,I2, ''         '',F6.1,''%'',F27.1,''%'')')
  ...
  110    CONTINUE
         ILINE=ILINE+6
         IF(VIB1.AND.DOPRNT)WRITE(6,'(/,'' VIBRATION'',I4)')K           CIO
         IF(VIB1.AND.DOPRNT)WRITE(6,'(  '' FREQ.    '',F8.2)')EIGS(K)   CIO
         IF(VIB2.AND.DOPRNT)WRITE(6,'(  '' T-DIPOLE '',F8.4)')DIPT(K)   CIO
         IF(VIB3.AND.DOPRNT)WRITE(6,'(  '' TRAVEL   '',F8.4)')TRAVEL(K) CIO
         IF(VIB4.AND.DOPRNT)WRITE(6,'(  '' RED. MASS'',F8.4)')REDMAS(K) CIO
         IF(ILINE.GT.52)THEN
            ILINE=0
            IF (DOPRNT) WRITE(6,'(''1'')')                              CIO
         ENDIF
  120 CONTINUE
      RETURN
 9999 RETURN 1                                                          CSTP
