      SUBROUTINE THERMO(A,B,C,LINEAR,SYM,WT,VIBS,NVIBS,ESCF,*)          CSTP
      IMPLICIT DOUBLE PRECISION (A-H,O-Z)
      DIMENSION VIBS(*)
      LOGICAL LINEAR
      CHARACTER*160 KEYWRD
      CHARACTER*80  KOMENT, TITLE, TMPKEY*160
...
C
     ...
     &                     ''TEMPERATURE RANGE STARTS TOO LOW,'',       CIO
     &                     '' LOWER BOUND IS RESET TO 30K'')')
            IT1=100
         ENDIF
         I=INDEX(TMPKEY,',')
         IF(I.NE.0) THEN
      ...
      ENDIF
   10 CONTINUE
      IF (DOPRNT) WRITE(6,'(//,A)')TITLE                                CIO
      IF (DOPRNT) WRITE(6,'(A)')KOMENT                                  CIO
      IF(LINEAR) THEN
         IF (DOPRNT) WRITE(6,'(//10X,''MOLECULE IS LINEAR'')')          CIO
      ELSE
         IF (DOPRNT) WRITE(6,'(//10X,''MOLECULE IS NOT LINEAR'')')      CIO
      ENDIF
      IF (DOPRNT) WRITE(6,'(/10X,''THERE ARE'',I3,                      CIO
     &'' GENUINE VIBRATIONS IN THIS '',                                 CIO
     &''SYSTEM'')')NVIBS
...
C   ***   OUTPUT SECTION   ***
     ...
     1                     ESCF+(HTOT-H298)/1000.D0,HTOT,CPTOT,STOT
         ...
         ENDIF
   80 CONTINUE
      IF (DOPRNT) WRITE(6,'(/3X,                                        CIO
     &'' * NOTE: HEATS OF FORMATION ARE RELATIVE TO THE'',              CIO
     &/12X,'' ELEMENTS IN THEIR STANDARD STATE AT 298K'')')
      RETURN
 9999 RETURN 1                                                          CSTP
      END
