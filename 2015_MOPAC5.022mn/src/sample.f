...
C   (PP/SS)
     ...
     4             +TDZ(KK)*TZ(LL)+TZ(KK)*TDZ(LL))
                        R(ISP)=G(3)*TX(KK)*TX(LL)+                       ! <===
     &                    G(4)*(TY(KK)*TY(LL)+TZ(KK)*TZ(LL))             ! <===
                     ENDIF
                  ELSEIF(MM.EQ.0) THEN
                     IF(LL.EQ.0) THEN
C   (SS/SP)
...
C   (SP/SP)
     ...
     4             +TDZ(LL)*TZ(NN)+TZ(LL)*TDZ(NN))
                        R(ISP)=G(6)*TX(LL)*TX(NN)+                       ! <===
     &                    G(7)*(TY(LL)*TY(NN)+TZ(LL)*TZ(NN))             ! <===
                     ELSE
C   (PP/SP)
     ...
     5                      +TDZ(KK)*TZ(NN)+TZ(KK)*TDZ(NN)))
                        R(ISP)=G(8)*TX(KK)*TX(LL)*TX(NN)+                ! <===
     &                    G(9)*(TY(KK)*TY(LL)+TZ(KK)*TZ(LL))*TX(NN)+     ! <===
     &                    G(10)*(TX(KK)*(TY(LL)*TY(NN)+TZ(LL)*TZ(NN))+   ! <===
...
C   (SS/PP)
     ...
     4              +TDZ(MM)*TZ(NN)+TZ(MM)*TDZ(NN))
                     R(ISP)=G(11)*TX(MM)*TX(NN)+                         ! <===
     &                 G(12)*(TY(MM)*TY(NN)+TZ(MM)*TZ(NN))               ! <===
                  ELSEIF(KK.EQ.0) THEN
C   (SP/PP)
     ...
     5                      +TDZ(NN)*TX(MM)+TZ(NN)*TDX(MM)))
                     R(ISP)=G(13)*TX(LL)*TX(MM)*TX(NN)+                  ! <===
     &                 G(14)*TX(LL)*(TY(MM)*TY(NN)+TZ(MM)*TZ(NN))+       ! <===
     &                 G(15)*(TY(LL)*(TY(MM)*TX(NN)+TY(NN)*TX(MM))+      ! <===
...
C      TO AVOID COMPILER DIFFICULTIES THIS IS DIVIDED
     ...
     8                  +TDZ(MM)*TY(NN)+TZ(MM)*TDY(NN)))
                   R(ISP)=G(16)*TX(KK)*TX(LL)*TX(MM)*TX(NN)+             ! <===
     &               G(17)*(TY(KK)*TY(LL)+TZ(KK)*TZ(LL))*TX(MM)*TX(NN)+  ! <===
     &               G(18)*TX(KK)*TX(LL)*(TY(MM)*TY(NN)+TZ(MM)*TZ(NN))+  ! <===
     &               G(19)*(TY(KK)*TY(LL)*TY(MM)*TY(NN)                  ! <===
     &                    +TZ(KK)*TZ(LL)*TZ(MM)*TZ(NN))+                 ! <===
     &               G(20)*(TX(KK)*(TX(MM)*(TY(LL)*TY(NN)+TZ(LL)*TZ(NN)) ! <===
     &                    +TX(NN)*(TY(LL)*TY(MM)+TZ(LL)*TZ(MM)))         ! <===
     &                    +TX(LL)*(TX(MM)*(TY(KK)*TY(NN)+TZ(KK)*TZ(NN))  ! <===
     &                    +TX(NN)*(TY(KK)*TY(MM)+TZ(KK)*TZ(MM))))        ! <===
     &               G(21)*(TY(KK)*TY(LL)*TZ(MM)*TZ(NN)                  ! <===
     &                    +TZ(KK)*TZ(LL)*TY(MM)*TY(NN))                  ! <===
     &               G(22)*(TY(KK)*TZ(LL)+TZ(KK)*TY(LL))                 ! <===
