      SUBROUTINE DELML2(COORD,I,J,NI,NJ,IA,ID,JA,JD,IX,RIJ,TOMB,ISP,*)  CSTP
...
CSAV         SAVE                                                           GL0892
      IF(NI.GT.1.OR.NJ.GT.1) CALL ROTAT(COORD,I,J,IX,RIJ,TOMB,2,*9999)   CSTP(call)
      IB=MAX(IA,ID)
      JB=MAX(JA,JD)
      DO 10 K=IA,IB
         KK=K-IA
...
C   (PP/SS)
     ...
     4             +TDZ(KK)*TZ(LL)+TZ(KK)*TDZ(LL))
                        UN(ISP)=G(3)*TX(KK)*TX(LL)+                      ! <=== LF1110
     &                    G(4)*(TY(KK)*TY(LL)+TZ(KK)*TZ(LL))             ! <=== LF1110
                     ENDIF
                  ELSEIF(MM.EQ.0) THEN
                     IF(LL.EQ.0) THEN
C   (SS/SP)
                        DR(ISP)=DG(5)*TX(NN)+G(5)*TDX(NN)
                        UN(ISP)=G(5)*TX(NN)                              ! <=== LF1110
                     ELSEIF(KK.EQ.0) THEN
C   (SP/SP)
     ...
     4             +TDZ(LL)*TZ(NN)+TZ(LL)*TDZ(NN))
                        UN(ISP)=G(6)*TX(LL)*TX(NN)+                      ! <=== LF1110
     &                    G(7)*(TY(LL)*TY(NN)+TZ(LL)*TZ(NN))             ! <=== LF1110
                     ELSE
C   (PP/SP)
     ...
     5                      +TDZ(KK)*TZ(NN)+TZ(KK)*TDZ(NN)))
                        UN(ISP)=G(8)*TX(KK)*TX(LL)*TX(NN)+               ! <=== LF1110
     &                    G(9)*(TY(KK)*TY(LL)+TZ(KK)*TZ(LL))*TX(NN)+     ! <=== LF1110
     &                    G(10)*(TX(KK)*(TY(LL)*TY(NN)+TZ(LL)*TZ(NN))+   ! <=== LF1110
     &                    TX(LL)*(TY(KK)*TY(NN)+TZ(KK)*TZ(NN)))          ! <=== LF1110
                     ENDIF
                  ELSEIF(LL.EQ.0) THEN
C   (SS/PP)
     ...
     4              +TDZ(MM)*TZ(NN)+TZ(MM)*TDZ(NN))
                     UN(ISP)=G(11)*TX(MM)*TX(NN)+                        ! <=== LF1110
     &                 G(12)*(TY(MM)*TY(NN)+TZ(MM)*TZ(NN))               ! <=== LF1110
                  ELSEIF(KK.EQ.0) THEN
C   (SP/PP)
     ...
     5                      +TDZ(NN)*TX(MM)+TZ(NN)*TDX(MM)))
                     UN(ISP)=G(13)*TX(LL)*TX(MM)*TX(NN)+                 ! <=== LF1110
     &                 G(14)*TX(LL)*(TY(MM)*TY(NN)+TZ(MM)*TZ(NN))+       ! <=== LF1110
     &                 G(15)*(TY(LL)*(TY(MM)*TX(NN)+TY(NN)*TX(MM))+      ! <=== LF1110
     &                 TZ(LL)*(TZ(MM)*TX(NN)+TZ(NN)*TX(MM)))             ! <=== LF1110
                  ELSE
C   (PP/PP)
...
C      TO AVOID COMPILER DIFFICULTIES THIS IS DIVIDED
     ...
     8                  +TDZ(MM)*TY(NN)+TZ(MM)*TDY(NN)))
                   UN(ISP)=G(16)*TX(KK)*TX(LL)*TX(MM)*TX(NN)+            ! <=== LF1110
     &               G(17)*(TY(KK)*TY(LL)+TZ(KK)*TZ(LL))*TX(MM)*TX(NN)+  ! <=== LF1110
     &               G(18)*TX(KK)*TX(LL)*(TY(MM)*TY(NN)+TZ(MM)*TZ(NN))+  ! <=== LF1110
     &               G(19)*(TY(KK)*TY(LL)*TY(MM)*TY(NN)                  ! <=== LF1110
     &                    +TZ(KK)*TZ(LL)*TZ(MM)*TZ(NN))+                 ! <=== LF1110
     &               G(20)*(TX(KK)*(TX(MM)*(TY(LL)*TY(NN)+TZ(LL)*TZ(NN)) ! <=== LF1110
     &                    +TX(NN)*(TY(LL)*TY(MM)+TZ(LL)*TZ(MM)))         ! <=== LF1110
     &                    +TX(LL)*(TX(MM)*(TY(KK)*TY(NN)+TZ(KK)*TZ(NN))  ! <=== LF1110
     &                    +TX(NN)*(TY(KK)*TY(MM)+TZ(KK)*TZ(MM))))+       ! <=== LF1110
     &               G(21)*(TY(KK)*TY(LL)*TZ(MM)*TZ(NN)                  ! <=== LF1110
     &                    +TZ(KK)*TZ(LL)*TY(MM)*TY(NN))+                 ! <=== LF1110
     &               G(22)*(TY(KK)*TZ(LL)+TZ(KK)*TY(LL))                 ! <=== LF1110
     &                    *(TY(MM)*TZ(NN)+TZ(MM)*TY(NN))                 ! <=== LF1110
                  ENDIF
   10 CONTINUE
      RETURN
 9999 RETURN 1                                                          CSTP
