      SUBROUTINE PERM(IPERM,NELS,NMOS,MAXMOS,NPERMS,*)                  CSTP
...
************************************************************************
      IF(NELS.GT.NMOS)THEN
         IF (DOPRNT) WRITE(6,'('' NUMBER OF PARTICLES,'',I3,            CIO
     &         '' GREATER THAN NO. '',''OF STATES,'',I3)')NELS,NMOS     CIO
         NPERMS=0
         RETURN
      ENDIF
      NPERMS=1
   ...
   40                                     IPERM(IADD(J),NPERMS)=1
     ...
     &            '' NUMBER OF PERMUTATIONS TOO GREAT, LIMIT 60'')')    CIO
                                             GOTO 60
                                          ENDIF
   50 CONTINUE
   60 NPERMS=NPERMS-1
