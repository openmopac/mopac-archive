      SUBROUTINE REPP(NI,NJ,RIJ,RI,CORE,*)                              CSTP
C***********************************************************************
C
...
C
C     ON INPUT RIJ     = INTERATOMIC DISTANCE (in ANGSTROMS)
...
C
C    ON OUTPUT RI      = ARRAY OF TWO-ELECTRON REPULSION INTEGRALS (in eV)
...
C***********************************************************************
      ...
      LOGICAL SI,SJ
      COMMON /MULTIP/ DD(120),QQ(120),ADD(120,3)
      COMMON /CORE/ TORE(120)
      COMMON /NATORB/ NATORB(120)
      COMMON /ESCALE/ ESCALE                                            ZP0111/LF0111
...
C     DEFINE CHARGE SEPARATIONS.
         ...
         QA=QQ(NI) * TD
         QB=QQ(NJ) * TD
C        Do "MOD4" keyword modification (excludes quadrupole terms).
         IF(PMODS(4)) THEN
            IF(HPUSED.AND.(NI.EQ.9).AND.(NJ.EQ.9)) QA = 0.0D0
            IF(HPUSED.AND.(NI.EQ.9).AND.(NJ.EQ.9)) QB = 0.0D0
         ENDIF
C
         AEE = PP/ADD(NI,1) + PP/ADD(NJ,1)
         AEE = AEE * AEE
...
c         write(6,*) "RI matrix:"                                             debug LF
c         write(6,*) (RI(NCOUNT),NCOUNT=1,22)                                 debug LF
C
...
C
         ...
         IF (PMODS(6).AND.HPUSED.AND.(NI.EQ.9).AND.(NJ.EQ.9)) THEN
C             Handle P Zhang's PMOv1a modifications if MOD6 keyword is used.
C             Note that keyword MOD6 will override keyword MOD2.
C             The old core-electron attraction correction by Peng for
C             the MOD6 keyword:
Cold              DUMPCORE = 1.D0 - 1.00D0*EXP(-1.1D0*RIJ*RIJ)              ZP0111/LF0111
C
C
C         *** IMPORTANT CHANGE from PMOv1 to PMO2 and beyond has been made for the MOD6
C                  keyword here!!!
C
C             Miho's new core-electron attraction correction for the
C             MOD6 keyword:
              DUMPCORE = 1.D0  - 1.00D0*EXP(-1.4D0*RIJ*RIJ)             MI0512
              CORE(3,1) = DUMPCORE*CORE(1,1)                            ZP0111/LF0111
              CORE(4,1) = DUMPCORE*CORE(1,1)                            ZP0111/LF0111
              CORE(3,2) = DUMPCORE*CORE(1,2)                            ZP0111/LF0111
              CORE(4,2) = DUMPCORE*CORE(1,2)                            ZP0111/LF0111
         ELSEIF (PMODS(2).AND.HPUSED.AND.(NI.EQ.9).AND.(NJ.EQ.9)) THEN
C             Handle P Zhang's PMOv1 modifications if MOD2 keyword is used.
              DUMPCORE = 1.D0 - 0.75D0*EXP(-1.1D0*RIJ*RIJ)              ZP0910
              CORE(2,1) = CORE(2,1)
              CORE(3,1) = CORE(1,1)*DUMPCORE
              CORE(4,1) = CORE(1,1)*DUMPCORE
              CORE(2,2) = CORE(2,2)
              CORE(3,2) = CORE(1,2)*DUMPCORE
              CORE(4,2) = CORE(1,2)*DUMPCORE
         ENDIF                                                          LF0910
C
      END IF
C
C#      write(6,*) "At end of REPP for NI,NJ,RIJ of:", NI,NJ,RIJ              debug LF
C#      write(6,*) "CORE is ",CORE                                            debug LF
      RETURN
C
 9999 RETURN 1                                                          CSTP
      END
