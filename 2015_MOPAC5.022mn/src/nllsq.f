...
C*
     ...
     &         '(/10X,''NUMBER OF CYCLES TO BE RUN ='',I5)')MXCYCL      CIO
      ENDIF
      TOLS1=1.D-12
      TOLS2=1.D-10
      TOLS5=1.D-6
...
C     **********
      IF(MIDDLE) THEN
         CALL PARSAV(0,N,M,*9999)                                       CSTP(call)
         NSCF=IIIUM(1)
         IF (DOPRNT) CLOSE(13)                                          CIO
         NCOUNT=IIIUM(5)
         MXCYCL=MXCYCL+ICYC
         DO 50 I=1,N
   50    X(I)=XLAST(I)
         ...
         GOTO 100
      ENDIF
      CALL COMPFG(X,.TRUE.,ESCF,.TRUE.,EFSLST,.TRUE.,*9999)             CSTP(call)
      IF( SCF1 ) GOTO 920
...
C     **********
  ...
  280 PN2 = PN2+P(I)**2
      IF(PN.LT.1.D-20) THEN
         IF(DOPRNT)                                                     CIO
     &     WRITE(6,'('' SYSTEM DOES NOT APPEAR TO BE OPTIMIZABLE.'',/   CIO
...
C     **********
      ...
      DO 300 I=1,N
         EFS(I)=0.D0
  300 XLAST(I)=X(I)
      CALL LOCMIN(M,X,N,P,SSQ,ALF,EFS,IERR,ESCF,*9999)                   CSTP(call)
...
C#      WRITE(6,730) SSQ
  ...
  630 CONTINUE
      ...
      TLEFT=TLEFT-TCYCLE
      IF(RESFIL)THEN
         IF (DOPRNT) WRITE(6,640)TLEFT,SQRT(SSQ),ESCF                   CIO
  640    FORMAT('  RESTART FILE WRITTEN,  TIME LEFT:',F12.5,
     1' GRAD.:',F10.3,' HEAT:',G14.7)
         RESFIL=.FALSE.
      ELSE
         IF (DOPRNT) WRITE(6,650)ICYC,TCYCLE,TLEFT,SQRT(SSQ),ESCF       CIO
  ...
  660    XLAST(I)=X(I)
         IIIUM(1)=NSCF
         CALL PARSAV(2,N,M,*9999)                                       CSTP(call)
      ENDIF
      IF(TLEFT .GT. TCYCLE*2) GO TO 100
  670 IIIUM(5)=NCOUNT
      DO 680 I=1,N
  680 XLAST(I)=X(I)
      IIIUM(1)=NSCF
      CALL PARSAV(1,N,M,*9999)                                          CSTP(call)
      RETURN 1                                                          CSTP (stop)
  690 IF (DOPRNT) WRITE (6,810)  NCOUNT                                 CIO
      GOTO 920
  700 IF (DOPRNT) WRITE (6,820)  NCOUNT                                 CIO
      GOTO 920
  710 CONTINUE
      IF (DOPRNT) WRITE(6,720) IXSO                                     CIO
