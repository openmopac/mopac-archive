...
c Common MOLKST splitted in MOLKSI and MOLKSR    Ivan Rossi 0394   &8)
     ...
     2               ,QD0(NUMATM),FD(NUMATM,NUMATM),QD(NUMATM)          GDH0997
      ...
      COMMON /CKDD/CK(105,105),DD(105,105)                              GDH0997
...
C
      ...
      QM(I)=ZERO                                                        GDH0997
      CALL SCOPY (NUMAT,ZERO,0,B(1,I),1,*9999)                          GDH0997 CSTP(call)
...
C     NET MULLIKEN CHARGES QM                                           GDH0997
 ...
 10   DD(I,J)=0.0
      NSQ=NUMAT*(NUMAT+1)/2
      IF(INDEX(KEYWRD,'AM1').NE.0 ) GOTO 150
      IF((INDEX(KEYWRD,'PM3').NE.0 ).AND.
     1  (INDEX(KEYWRD,'PM3-D').EQ.0)) GOTO 200                          LF0309
      RETURN
 150  CONTINUE
