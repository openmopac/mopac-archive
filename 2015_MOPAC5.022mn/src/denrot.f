...
***********************************************************************
   ...
   30       PAB(KK,LL)=P(L+(K*(K-1))/2)
     ...
     1                 XYZ(1,J),XYZ(2,J),XYZ(3,J),IPQ,JPQ,C,R,*9999)     CSTP(call)
            DO 40 I1=1,9
               DO 40 J1=1,9
   40       AROT(I1,J1)=0.D0
            DO 50 I1=1,35
...
C
  ...
  180 LL=0
      ...
      MA=2*M+1
      M=NA+M-1
      IF (DOPRNT) WRITE(6,'(/16X,10(1X,A7,3X))')(ITEXT(I),I=NA,M)       CIO
      IF (DOPRNT) WRITE(6,'(15X,10(2X,A2,I3,4X))')(JTEXT(I),NATOM(I),   CIO
     &                  I=NA,M)                                         CIO
      IF (DOPRNT) WRITE (6,'(20A6)') (LINE(K),K=1,MA)                   CIO
      DO 200 I=NA,IPRT
         LL=LL+1
         K=(I*(I-1))/2
         L=MIN0((K+M),(K+I))
         ...
         IF (DOPRNT) WRITE(6,'(/17X,10(1X,A7,3X))')(ITEXT(N),N=NA,M)    CIO
         IF (DOPRNT) WRITE(6,'( 17X,10(2X,A2,I3,4X))')(JTEXT(N),        CIO
     &                     NATOM(N),N=NA,M)                             CIO
         IF (DOPRNT) WRITE (6,'(20A6)') (LINE(N),N=1,MA)
         KK=4
         LL=0
  190    IF (DOPRNT) WRITE (6,'(1X,A7,1X,A2,I3,10F11.6)')               CIO
     1   ITEXT(I),JTEXT(I),NATOM(I),(B(N),N=K,L)                        CIO
  200 CONTINUE
