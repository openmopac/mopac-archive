...
C******************************************************                 
      ...
      IF(FIRST) THEN
          FIRST=.FALSE.
C          CALL DEFINE_EVENT_FLAG_CLUSTER
      ENDIF
      CALL TIMCLK(CPU)                                                  
C      CALL CHECK_EVENT_FLAG(64,SET)
      IF(SET)   THEN
      CPU=CPU+1.D6
      IF( SETOK) THEN
...
C                                                                       
     ...
     2 ZERO4                                                            
      ...
      INTEGER*4 ZERO,ZERO1,ZERO2,ZERO3,ZERO4             
C                                                                       
...
C       ==================================================              
      ...
      PFLTS_ADR       = %LOC(PFLTS)                                     
C                                                                       
C      IF (.NOT. SYS$GETJPI(,,,LEN4A,,,)) THEN                           
C          WRITE(ITTY,*) 'ERROR FROM SYS$GETJPI'                         
C          ENDIF                                                         
C                                                                       
      RETURN                                                            
C                                                                       
