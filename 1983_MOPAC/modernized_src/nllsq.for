...
C****************************************************************************
C
C   NLLSQ IS A NON-DERIVATIVE, NONLINEAR LEAST-SQUARES MINIMIZER. IT USES
C         BARTEL'S PROCEDURE TO MINIMISE A FUNCTION WHICH IS A SUM OF
C         SQUARES.
C
C     ON INPUT N    = NUMBER OF UNKNOWNS
C              X    = PARAMETERS OF FUNCTION TO BE MINIMIZED.
C
C     ON EXIT  X    = OPTIMISED PARAMETERS.
C
C     THE FUNCTION TO BE MINIMIZED IS "COMPFG". COMPFG MUST HAVE THE CALLING
C     SEQUENCE     "CALL COMPFG(XPARAM,.TRUE.,ESCF,.TRUE.,EFS,.TRUE.)"
C                   SSQ=DOT(EFS,EFS,N)
C     WHERE   EFS  IS A VECTOR WHICH  COMPFG  FILLS WITH THE N INDIVIDUAL
C                  COMPONENTS OF THE ERROR FUNCTION AT THE POINT X
C             SSQ IS THE VALUE OF THE SUM OF THE  EFS  SQUARED.
C     IN THIS FORMULATION OF NLLSQ M AND N ARE THE SAME. 
C     THE PRECISE DEFINITIONS OF THESE TWO QUANTITIES IS:
C
C      N = NUMBER OF PARAMETERS TO BE OPTIMIZED.
C      M = NUMBER OF REFERENCE FUNCTIONS. M MUST BE GREATER THEN, OR EQUAL TO, N
C****************************************************************************
...
C      TOLS3 = RELATIVE TOLERANCE ON SSQ OVERALL
C      TOLS4 = ABSOLUTE TOLERANCE ON SSQ OVERALL
