...
!
      ...
      in_house_only, no_pKa, nalpha_open, nbeta_open, use_ref_geo, N_Hbonds, &
      n_methods, methods, methods_keys, hpress, nsp2_corr, Si_O_H_corr, sum_dihed, atheat, &
      prt_gradients, prt_coords, prt_cart, prt_pops, prt_charges
!
...
!-----------------------------------------------
      ...
      dip, dumy(3), pKa_unsorted(numat), distortion, rms
      logical :: ci, lprtgra, still, bcc, opend
      character  :: type(3)*11, idate*24, gtype*13, grtype*14, &
      flepo(17)*58, iter(2)*58, caltyp*27, namfil*241
...
!
        ...
        do i = 1, numat 
          ...
          if (prt_charges) then
            if (maxtxt == 26) then
              write (iw, "(I5,9X,A30,4X,F15.6,F14.4,3F12.5)") i, elemnt (l)//"("//txtatm(i)//")", &
                 & q(i), q2(i), rxyz(1:j)
            else
        ...
        if (nelecs /= 0) then 
          ...
          if (prt_pops) then
            if (mozyme) then
              ...
              if (maxtxt > 1) then
                do i = 1, numat
                  write (iw, '(i5,1x,a, 9f10.5)') i, elemnt(nat(i))//"("//txtatm(i)(:maxtxt)//")", &
              ...
              if (maxtxt > 1) then
                do i = 1, numat
                  write (iw, '(2x, a, 9f10.5)') elemnt(nat(i))//"("//txtatm(i)(:maxtxt)//")", &
