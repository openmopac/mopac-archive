...
!
      ...
      p, pa, pb, na, labels, loc, time_start, l_atom, coord, txtatm, coorda, &
      txtatm1
!
      USE molkst_C, only : gnorm, natoms, numat, nvar, numcal, nscf, id, &
        escf, iflepo, iscf, keywrd, last, moperr, maxatoms, ncomments, &
        time0, atheat, errtxt, isok, mpack, gui, line, na1, refkey, &
        ...
        num_threads, nl_atoms, use_ref_geo, prt_coords
!
...
!
      USE chanel_C, only : ir, iw, iarc, output_fn, end_fn, iend, &
        archive_fn, log, ilog, xyz_fn, job_fn
!
...
!
      ...
      maxatoms = natoms
!
!   Unique definition of in_house_only
!
      inquire (file ="M:\Survey_Of_Solids\bits.txt", exist = in_house_only)
   ...
   10 continue
      ...
      if (numcal > 1 .and. numcal < 4 .and. index(keywrd," GEO_DAT") /= 0) then
!
...
!
      i = numcal
      call readmo
      if (moperr .and. numcal == 1 .and. index(keywrd," GEO_DAT") == 0) goto 100    
      if (moperr) goto 101
      if (numcal == 1) then
...
!
      ...
      if (numat > 46000) then
        write(line,'(a,i5,a)')" Data set '"//trim(jobnam)//"' exists, but is much too large to run."
        write(0,'(//10x,a,//)')trim(line)
...
!
      ...
      sparkles = .false.
      do i = 1, natoms
        if (labels(i) > 57 .and. labels(i) < 71) then
          sparkles = .true.
          exit
        end if
      end do
...
!
      ...
      if (index(keywrd,' 0SCF') + index(keywrd, " RESEQ") /= 0 ) then
        ...
        rewind iarc
        if (index(keywrd,' 0SCF') /= 0 .and. index(keywrd, " MINI") /= 0) then
      ...
      if ( index(keywrd," PDBOUT") /= 0 .and. maxtxt /= 26 .and. index(keywrd," RESID") == 0) then
        if (maxtxt == 0) then
          ...
          txtatm1(:natoms - id) = txtatm(:natoms - id)
        else
...
!
          if (index(keywrd, " NORES") == 0 .and. (maxtxt == 26 .or. index(keywrd, " RESID") /= 0) &
          ...
          if (index(keywrd, " SITE=") + index(keywrd, " ADD-H") /= 0) then
            ...
            if (index(keywrd, " ADD-H") /= 0) then
              ...
              do j = 1, 6
                ...
                i = index(line, " ADD-H")
                if (i > 0) then
                  refkey(j) = refkey(j)(:i)//trim(refkey(j)(i + 6:))
                  exit
                end if
              end do
...
!
      ...
      if (maxtxt == 26) call compare_txtatm(moperr, moperr)
      if (moperr) then
...
!
      ...
      if (index(keywrd,' SADDLE') /= 0) then
        ...
        call powsq ()
      else if (index(keywrd,' 1SCF') /= 0 .or. nvar == 0) then
        iflepo = 1
        iscf = 1
        last = 1
        i = index(keywrd,' GRAD')
      ...
      else if (index(keywrd, " REFINE-TS") /= 0) then
    !    call Refine_TS_for_Proteins
        ...
        id == 0 .and. index(keywrd, " GEO_REF") /= 0 .or. &
        nvar > 1000 .and. index(keywrd,' EF') == 0) then
  ...
  100 continue
      ...
      if ( .not. gui) then
        ...
        if (index(keywrd," GEO_DAT") /= 0) then
          ...
          write(line,'(a)')"GEO_DAT="//keywrd(i:j)          
