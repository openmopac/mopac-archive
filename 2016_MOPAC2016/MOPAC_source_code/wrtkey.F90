...
!
  ...
  if (myword(allkey, " NORES"))  write (iw, '(" *  NORESEQ    - DO NOT RESEQUENCE Z-MATIX INTO NORMAL PDB FORMAT")')
  ...
  if (i > 1) then
    j = index(keywrd(i:),")") 
    if (j == 0) then
      write (iw, '(" ***CHAIN must be followed by open and close parentheses, e.g., CHAIN=(ABCD)")')
      call web_message(iw,"chain.html")
      call mopend("Keyword CHAIN not properly defined")
      return
    end if
    j = j + i
    allkey(i:j) = " "
                                 write (iw, '(" *  CHAIN      - PDB CHAIN LETTERS EXPLICITLY DEFINED")')
                                 write (iw, '(" *  Keyword:   ",a)')keywrd(i:j)
  end if     
  ...
  if (i /= 0) then
    ...
    allkey(i:i + 7) = " "
                                 write (iw, '(" *  SETUP      - EXTRA KEYWORDS TO BE READ FROM FILE '''//trim(line)//'''")')
  else
  ...
  if (myword(allkey, " SITE=") .or. myword(allkey, " SITE(") )   then
    ...
    if (id /= 0) then
       write(iw,'(1x,a)')"*","*","***Keyword SITE=(An(x)) can only be used for systems without Tv","*","*"
  ...
  if (index(allkey, " OUTPUT") > 0) then
    ...
    if (i > 1) then
      ...
      prt_topo      = (index(line, "T") /= 0) 
                      write (iw,'(" *  MOPAC      - USE OLD MOPAC CONVENTION FOR FIRST THREE ATOMS")')
...
subroutine wrtwor (allkey)
  ...
  if (myword(allkey, " LOCATE-TS"))  write (iw,'(" *  LOCATE-TS  - LOCATE A TRANSITION STATE IN A PROTEIN MECHANISM")')
  if (myword(allkey, " REFINE-TS"))  write (iw,'(" *  REFINE-TS  - REFINE A TRANSITION STATE IN A PROTEIN MECHANISM")')
    ...
    if (index(keywrd, " RESEQ") /= 0) then
      ...
      if (index(keywrd, " NORESEQ") == 0 .or. ii <= natoms - id) then
        ...
        if (ii <= natoms - id) then
          write(iw,'(/2x,a, i6, a)')" Atom:",j, " is a hydrogen atom."
          write(iw,'(2x,a, i6, a)')" Atom:",ii, " is a non-hydrogen atom with a higher atom number than the hydrogen atom."
