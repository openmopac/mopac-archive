...
!
    ...
    implicit none
    integer :: io_stat, i, njobs, j, counter, nprogs, k, now, previous, &
      day, hour, minute, second, itim = 8
...
!
    read(time,'(8x,i2,x,i2,x,i2)') day, hour, minute
    j = 0
    do i = 1, 12
      if (index(time, mon(i)) /= 0) exit
      j = j + dim(i)
    end do
    if (debug) write(*,*)"month:", i, "day:", day, "hour:", hour, "minute:", minute
 ...
 97     call sleepqq(1000)
      ...
      do j = 1, 2
        ...
        if (debug) write(*,'(a)') '"'//trim(line)//'"'
        if (debug) write(*,*) io_stat
        if (io_stat == 0) exit
...
!
      ...
      time = line(k + 4:)
      if (debug) write(*,'(2a)')"<file>.out last changed:", "'"//trim(line)//"'"
      if (debug) write(*,'(2a)')"<file>.out last changed:", "'"//trim(time)//"'"
      read(time,'(i2,x,i2,x,i2)') day, hour, minute
      if (debug) write(*,*)"month:", i, "day:", day, "hour:", hour, "minute:", minute
      previous = 1440*(j + day) + 60*hour + minute 
      if (debug) write(*,*)"Now, .out file:", now, previous
      do 
...
!
      ...
      if ( .not. exists) open(unit = 9, name = trim(job_ref)//".okay")
    else
!
!  kill
!
      ...
      if (exists) then
        open(unit = 9, name = trim(job_name)//".okay")
