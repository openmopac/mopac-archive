...
!  otherwise wait six seconds and check again.
!
...
!
 !   use dfport, only : system
    ...
    integer :: maxprog, itim=7, io_stat, i, njobs, j, counter, nprogs, k, confirm = 0, delay = 0, &
      delay_200 = 200    
    logical :: windows
    ...
    counter = (ichar(idate(12:12)) - ichar("0"))*100000 + &
              (ichar(idate(13:13)) - ichar("0"))*10000  + &
              (ichar(idate(15:15)) - ichar("0"))*1000   + &
              (ichar(idate(16:16)) - ichar("0"))*100    + &
              (ichar(idate(18:18)) - ichar("0"))*10     + &
              (ichar(idate(19:19)) - ichar("0"))        
    do
      counter = counter + 1
      write(location,'(a,i6.6,a)',iostat = i)trim(tmp_file)//"jobs_running",counter,".lst"
      if (windows) then
!
...
!
      ...
      do
        open(unit=itim,file=trim(location), err = 99, iostat = io_stat)
        if (io_stat == 0) exit
        call sleepqq(delay_200)
      end do
      njobs = 0
      j = 0
      do
        ...
        if (io_stat /= 0) then
          if (index(line,"END") == 0)  njobs = 1000
          call sleepqq(delay_200)
          exit
        end if
        do i = 1, nprogs
