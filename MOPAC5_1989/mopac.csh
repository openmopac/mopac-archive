...
alias ddat   'ls -tl *.dat |  more '
alias dout   'ls -tl *.out |  more '
alias darc   'ls -tl *.arc |  more '
alias dres   'ls -tl *.res |  more '
alias dden   'ls -tl *.den |  more '
#
...
if !(-e $mopacdir/.qjobs) touch       $mopacdir/.qjobs
...
nextone:
if !(-e $mopacdir/.qlock) touch       $mopacdir/.qlock
echo queue is running >> $mopacdir/.qlock
#
#  do a cleanup
     $mopacdir/mopac.csh cleanup
#
...
set oldens  = `head -3 $job.dat | grep -i "oldens" | wc -l`
if($oldens != 0) then
 if !(-e $job.den) goto nooldensq
  else
  foreach file (res den)
...
foreach file (out log esp arc syb end)
    if -e $job.$file mv $job.$file $job.$file.$$
  end
#
...
setenv FOR018 $job.brz
setenv FOR020 $job.ump
setenv FOR021 $job.mep
...
nice +8 $mopacdir/$smallmopac $job >& $job.err_log 
...
check:
      set nobig = `ps ax | grep "$mopacdir" | grep "/$bigmopac" | wc -l`
        ...
        if $nobig != 0 goto check
            rm $job.out
            rm $job.log
            time $mopacdir/$bigmopac $job  > $job.log
    endif
endif
#chown $owner $job.*
if !(-e $mopacdir/.qdone) touch       $mopacdir/.qdone
...
#
nooldensq:
#  A nextrun has been specified but the  .den file does not exist
echo 'I cannot find the "$job".den.  Please help'
if !(-e $mopacdir/.qdone) touch       $mopacdir/.qdone
echo $job is not done >> $mopacdir/.qdone
#
goto nextrun 
...
# 
#
if !(-e $mopacdir/.qjobs2) touch       $mopacdir/.qjobs2
...
#
nooldensm:
#  Oldens has been specified but the .den file does not exist
echo 'I cannot find the "$job".den.  Please help'
#
exit
norestartm:
...
#
#nice +5 time $mopacdir/$smallmopac $job 
nice +5 time $mopacdir/$smallmopac $job >& $job.err_log  
set iok1 = `tail -1 "$job.out" | grep "IN SYSTEM" | wc -l`
set iok2 = `tail -1 "$job.out" | grep "ATOMS ALLOWED" | wc -l`
set iok3 = `tail -1 "$job.out" | grep "MAXIMUM ALLOWED" | wc -l`
set iok = `expr "$iok1" + "$iok2" + "$iok3" `
...
exit
shut_ok:
if !(-e $rname.end ) touch       $rname.end 
