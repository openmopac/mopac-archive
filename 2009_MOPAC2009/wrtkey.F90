...
subroutine wrtchk (allkey)
  ...
  integer :: i, j, l
  character :: ch
  character (len=241), intent (inout) :: allkey
...
!
  ...
  if (mozyme) then
    if (index(keywrd, " UHF") /= 0) then
      write(iw,"(a)")" Keyword UHF cannot be used with MOZYME"
      call mopend(" Keyword UHF cannot be used with MOZYME")
    end if
    if (index(keywrd, " ENPART") /= 0)  then
      write(iw,'(/,10x,a,/)')" Keyword ENPART is not available with MOZYME"
      call mopend(" Keyword ENPART is not available with MOZYME")
    end if  
    if (index(keywrd, " LOCAL") /= 0)  then
      write(iw,'(/,10x,a,/)')" Keyword LOCAL is not available with MOZYME"
    ...
    if (index(keywrd," RE-LOC") /= 0) then
      ...
      if (index(keywrd(i:j), "=")  + index(keywrd," DENOUT") + &
        ...
        write(iw,'(/,10x,a,/)')" Keyword RE-LOC only has meaning if DENOUT or VECTORS or ALLVECTORS is present"
        call mopend(" Keyword RE-LOC only has meaning if DENOUT or VECTORS or ALLVECTORS is present")
      end if                                               
    end if  
    if (id /= 0) then
      if (index(keywrd, " CUTOF") /= 0 ) then
        write(iw,"(//10x,a,//)")" CUTOFx=n.nn -type keywords do not work with MOZYME for infinite systems"
    ...
    if (index(keywrd," 1SCF") /= 0 .and. index(keywrd," RAPID") /= 0) then
      write(iw,"(//10x,a,//)")" RAPID cannot be used with 1SCF"
...
!
  if (uhf) then
    if (rhf) then
      write (iw, "(//10X,'UHF and RHF cannot both be used')")
...
!
      ...
      write (iw, "(//10X,'POLAR does not work with UHF')")
...
!
  if (Index (keywrd, " INT ") /= 0 .and. Index (keywrd, " XYZ") /= 0) then
    write (iw,*) " INT cannot be used with XYZ"
...
!
    ...
    if (method_rm1)     i = i + 1
    if (i > 1) then
      write (iw, '(//10 x, " ONLY ONE OF MNDO, MNDOD, AM1, PM3, RM1, AND PM6 ALLOWED")')            
      write (iw, 10000)
10000 format(/ / 10 x, " IMPOSSIBLE OPTION REQUESTED")
      ...
      return
       ...
       !
      i = 0
      if (Index (keywrd, " FORCE") /= 0)   i = i + 1
      if (Index (keywrd, " BFGS") /= 0)    i = i + 1
      if (Index (keywrd, " LBFGS") /= 0)   i = i + 1
      if (Index (keywrd, " EF") /= 0)      i = i + 1
      if (Index (keywrd, " TS") /= 0)      i = i + 1
      if (Index (keywrd, " SIGMA") /= 0)   i = i + 1
      if (Index (keywrd, " NLLSQ") /= 0)   i = i + 1
      if (i > 1) then
10020     format (//10 x, "MORE THAN ONE GEOMETRY OPTION HAS BEEN ", &
       & "SPECIFIED",/10 x, "CONFLICT MUST BE RESOLVED BEFORE JOB WILL RUN")
        write (iw, 10020)
...
!
   if (allkey /= ' ' .and. .not. is_PARAM) then 
        ...
        if (l /= 0) then 
          ...
          write (iw, "(///10X,'UNRECOGNIZED KEY-WORDS: (',A,')')") allkey (:j)
          call to_screen('UNRECOGNIZED KEY-WORDS: ')
          call to_screen(allkey (:j))
          write (iw, &
        &"(///10x,'IF THESE ARE DEBUG KEYWORDS, ADD THE KEYWORD ""DEBUG""')")
          call mopend ("IF THESE ARE DEBUG KEYWORDS, ADD THE KEYWORD ""DEBUG"".")
        end if
      return
    end if
1020 if (is_PARAM) return
  write (iw, "(//10X,' CALCULATION ABANDONED, SORRY!')")
...
subroutine wrtcon (allkey)
  ...
  use reada_I
  implicit none
  character (len=241), intent (out) :: allkey
  double precision :: epsi
  integer :: i, ielec, ilevel, j, k, l
  mozyme = (index(keywrd," MOZ") + index(keywrd," LEWIS") + &
    index(keywrd, " RESEQ") + index(keywrd, " CHARGES") /= 0)   
...
!
  ...
  if (myword(allkey, " PM5 "))   write (iw, '(" *  PM5      - This Hamiltonian is not supported")')
  if (myword(allkey, " BIRAD"))  write (iw, '(" *  BIRADICAL- SYSTEM HAS TWO UNPAIRED ELECTRONS")')
  if (myword(allkey, " EXCIT"))  write (iw, '(" *  EXCITED  - FIRST EXCITED STATE IS TO BE OPTIMIZED")')
  if (myword(allkey, " VELO"))   write (iw, '(" *  VELOCITY - INPUT STARTING VELOCITIES FOR DRC")')
  if (myword(allkey, " GEO-OK")) write (iw, '(" *  GEO-OK   - OVERRIDE INTERATOMIC DISTANCE CHECK")')
  if (myword(allkey, " PM6-D")) then 
    ...
    elseif (index(keywrd, "PM6-D3") /= 0) then
      write (iw, '(" *  PM6-D3   - CORRECT DISPERSION AND HYDROGEN BOND TERMS (S.Grimme, unpublished")')
  ...
  if (myword(allkey, " RESEQ"))  write (iw, '(" *  RESEQ    - RESEQUENCE Z-MATIX INTO NORMAL PDB FORMAT")')
  ...
  if (myword(allkey, " CHAIN"))  write (iw, '(" *  CHAINS   - PDB CHAIN LETTERS EXPLICITLY DEFINED")')
  ...
  if (myword(allkey, " SETPI"))  write (iw, '(" *  SETPI    - SOME OR ALL PI BONDS EXPLICITELY SET BY USER")')
  ...
  if (myword(allkey, " GEO_REF")) then
    ...
    if (index(keywrd(i+10:j - 1), "/") + index(keywrd(i+10:j - 1), "\") == 0) then
      write(iw,'(/10x,a)')" GEO_REF requires the reference data set path to be used"
      if (verson(7:7) == "W") then
        write(iw,'(10x,a)')" e.g. geo_ref=""C:\myjobs\"//keywrd(i+10:j - 1)//'"'
      else
        write(iw,'(10x,a)')" e.g. geo_ref=""/Users/myname/myjobs/"//keywrd(i+10:j - 1)//'"'
      end if
      call web_message(iw,"geo_ref.html")
      call mopend("Location of reference file defined by keyword GEO_REF missing")
    end if
    j = j + 1
    k = j
    if (allkey(j:j) /= " ") then                               
  ...
  if (myword(allkey, " RESI"))   write (iw, '(" *  RESIDUES - DETERMINE THE SEQUENCE OF RESIDUES")')
  if (myword(allkey, " NORES"))  write (iw, '(" *  NORES    - DO NOT DETERMINE THE SEQUENCE OF RESIDUES")')
  if (myword(allkey, " SITE"))   write (iw, '(" *  SITE     - SET IONIZATION LEVELS OF IONIZABLE RESIDUES ")')
...
!
  if (myword(allkey, " CHARGE=")) &
    write (iw,'(3(" *", /), " *", 15 x, "  CHARGE ON SYSTEM =", i6, 3 (/, " *"))') &
...
!
 ...
 if (myword(allkey, " STEP"))  then
   write (iw,'(" *  STEP     - STEP-SIZE IN PATH =", f7.2)') &
   reada (keywrd, Index (keywrd, "STEP")+5)
   if (index(keywrd, " POINT") == 0) then
     write (iw,'("*",/," *           - **** KEYWORD POINT MISSING ****",/,"*")') 
     call mopend (" KEYWORD POINT MISSING")
   end if
   i = 1
 end if
 if (i > 0) then
...
!
   ...
   if(i /= k) then
     write(iw,*)" The number of optimization flags set to '-1' does not match the keywords used"
...
subroutine wrtout (allkey)
  use molkst_C, only : keywrd, mozyme
  ...
  if (myword(allkey, " AUX"))     write (iw,'(" *  AUX      - OUTPUT AUXILIARY INFORMATION")')
...
10411 format (" *             THIS KEYWORD CANNOT BE USED IN THIS VERSION OF MOPAC.")
  ...
  log = .false.
  if (myword(allkey, " LOG")) then
    iw0 = 0
    log = .true.
    write (iw,'(" *  LOG      - GENERATE A LOG FILE")')
  end if
...
subroutine wrtwor (allkey)
  use molkst_C, only: tleft, tdump, keywrd
  use chanel_C, only: iw
  use myword_I
  use reada_I
  implicit none
  ...
  if (myword(allkey, " T=")) then
    ...
    do j = i + 3, 241
      if (j == 241 .or. keywrd(j+1:j+1) == " ") go to 1000
    end do
...
1020 time = 1.0d0
  ...
  if (myword(allkey, " DUMP")) then
    ...
    do j = i + 6, 241
      if (j == 241 .or. keywrd(j+1:j+1) == " ") go to 1030
    end do
...
!
  ...
  if (myword(allkey, " METAL")) then
    ...
    ii = max(15 + i - j, 1)
    write (iw, '(" *  ", a, "- ELEMENTS DEFINED AS FULLY IONIZED METALS FOR LEWIS STRUCTURE")') &
...
10880 format (" *  GNORM=   - EXIT WHEN GRADIENT NORM DROPS BELOW ", g8.3)
    write (iw, 10880) reada (keywrd, Index (keywrd, " GNORM"))
  end if
  if (myword(allkey, " DELTA")) then
10881 format (" *  DELTA=   - EXIT WHEN ENERGY DIFFERENCE DROPS BELOW ", g8.3)
...
!
  if (myword(allkey, " RELSCF")) then
      write (iw, 10920) reada (keywrd, Index (keywrd, " RELSCF")), 1.d-4
    end if
