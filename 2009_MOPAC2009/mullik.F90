...
!  THE  +1.D-14 IS TO PREVENT POSSIBLE ERRORS IN THE DIAGONALIZATION.
      ...
      call rsp (h, norbs, norbs, eig, vecs) 
      do i = 1, norbs 
      ...
      if (graph) then 
        if (graph_formatted) then
          write(igpt,"(i5, a)") numat," MOPAC-Graphical data Version 2009."//verson
          do i = 1, numat
...
!
      ...
      i = -1 
      call densit (vecs, norbs, norbs, nclose, 2.d0, nopen, fract, pb, 2) 
      pb = pb*store
      summ = 0.D0 
      do i = 1, norbs 
        sum = 0 
