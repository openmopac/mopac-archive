...
!
      ...
      & uhf, method_am1, method_pm3, method_mndo, id, msdel, mol_weight, &
      is_PARAM, formula, ispd, mozyme, nvar, rhf, method_pm6, old_chrge, &
      N_3_present, Si_O_H_present, nalpha_open, nbeta_open, method_PM7
!
...
!
      ...
      call set_up_dentate     
      if (id == 0) then
        if (.not. mozyme .or. numat < 200) then
      ...
      if (.not. rhf .and. .not. uhf) then
        if (mod(nelecs,2) == 0) then
          ...
          if (index(keywrd," POLAR") /= 0) then
            ...
            call mopend(" POLAR used with odd-electron system.  Add keyword RHF and re-run")
            return
          end if
        end if
      end if
      ...
      '(2/10X,''SYSTEM SPECIFIED WITH ODD NUMBER OF ELECTRONS, CORRECT FAULT '')') 
        ...
        if (ielec /= 0) then 
          ...
          if (nclose + nopen > norbs) then 
            write (iw, '(A)') ' NUMBER OF DOUBLY FILLED PLUS PARTLY FILLED LEVELS' 
       ...
       'NUMBER OF DOUBLY FILLED PLUS PARTLY FILLED LEVELS GREATER THAN TOTAL NUMBER OF ORBITALS') 
        ...
        if (mode /= 1 .and. .not. mozyme) then
          write (iw, '(2/6X,''RHF CALCULATION, NO. OF DOUBLY OCCUPIED LEVELS ='',I5,/)') nclose 
...
!
  ...
  !    Si_O_H_present = .false.
      ...
      if (mode /= 1 .and. ii>1 .and. .not. (method_pm6 .or. method_PM7)) then 
        write (iw, '(A,I4,2A)') ' THERE ARE', ii/2, &
          ' PEPTIDE LINKAGES IDENTIFIED IN THIS SYSTEM' 
        write (iw, '(A)') &
      ' IF YOU WANT MM CORRECTION TO THE CONH BARRIER, ADD THE KEY-WORD "MMOK"' 
      else if (mode /= 1 .and. nnhco /= 0 .and. .not. is_PARAM) then 
        if (index(keywrd,'MMOK') /= 0) then 
          write (iw, '(A)') &
            ' MOLECULAR MECHANICS CORRECTION APPLIED TO PEPTIDE LINKAGE' 
        else        
          if (.not. (method_pm6 .or. method_PM7)) then
            write (iw, '(A)') ' YOU CAN SPECIFY "NOMM" OR "MMOK" REGARDING MOLECULAR MECHANICS CORRECTION'   
            write (iw, '(10x,A, i5, a)') ' THIS SYSTEM CONTAINS',nnhco/2,' -HNCO- GROUPS.' 
          end if
          nnhco = 0 
        endif 
      endif            
      ...
      if (rmin<0.2D0 .and. index(keywrd,'GEO-OK')==0 .or. rmin < 1.d-4) then 
        icount = 0 
        ireal = iminr 
        jreal = jminr 
        do i = 1, natoms 
  ...
  250   format(/,/,'   ATOMS',i6,' AND',i6,' ARE SEPARATED BY',f8.4,&
...
subroutine setcup 
   ...
   !
   !***********************************************************************
    use molkst_C, only: cutofp, id, keywrd, l1u, l2u, l3u, l123, l11, l21, l31
   ...
   !
    if (id == 1) then  !   Polymer case
      ...
      if (tv1 < 1.d0) then
      call mopend ("  Length of translation vector is too small.")
      return
      end if
      ...
      if (r1 < 1.d0) call mopend ("  Length of first translation vector is too small.")
      if (r2 < 1.d0) call mopend ("  Length of second translation vector is too small.")
      if (r3 < 1.d0) call mopend ("  Length of third translation vector is too small.")
        return
      end if
      r12 = Sqrt ((tvec(1, 2)-tvec(1, 1))**2 + (tvec(2, 2)-tvec(2, 1))**2 &
           & + (tvec(3, 2)-tvec(3, 1))**2)
...
subroutine write_cell(iprt)
  use molkst_C, only: mol_weight, escf, numat, title, keywrd, gnorm, &
   line, mers, gui
  use funcon_C, only: a0, ev, fpc_9, fpc_10
  use common_arrays_C, only: nat, tvec
  use chanel_C, only: archive_fn
  use ef_C, only : nstep
  use reada_I
  use volume_I
  implicit none
...
!
  ...
  j = len_trim(line) - 4 !  Remove the ".arc"
  do i = 2, j
...
!
              xi = dot(dsum, dsum1,3)/sqrt(dot(dsum1,dsum1,3))
!
!  Convert this into a pressure = force per unit area
!
              xi = -(4184.d0*10.d0**30)/fpc_10 * xi*sqrt(dot(dsum1,dsum1,3))/volume (tvec, 3)
              ...
              if (i2 == 0) then
                ...
                if (iprt == 0) then
                  call to_screen(line(:len_trim(line)))
                else
                  write(iprt,*)line(:len_trim(line))
                end if   
                i2 = 1
              end if
