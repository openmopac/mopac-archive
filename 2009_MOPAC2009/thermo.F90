...
!
        ...
        if (it1 < 100) then 
          write (iw, &
      '(2/10X,''TEMPERATURE RANGE STARTS TOO LOW, LOWER BOUND IS RESET TO 30K'')') 
          it1 = 100 
        endif 
        i = index(tmpkey,',') 
        if (i /= 0) then 
   ...
   20 format(10x,'THIS THERMODYNAMICS CALCULATION IS LIMITED TO',/,10x,&
      ...
      if (ilim == 2 .and. Abs(trange(1) - trange(2)) < 1.d-4) ilim = 1
      do ir = 1, ilim 
        itemp = int(trange(ir)) 
        T = itemp 
...
!     UPDATED BY PROF. HIRANO
        ...
        write (iw, '(9X,''TOT.'',12X,F17.3,F13.4,2F11.4)') escf + (htot - h298)&
          /1000.D0, htot, cptot, stot 
      end do 
      if (ilim == 1 .and. Abs(trange(1) - 298) < 1.d1) then
        cp298 = cptot
        s298  = stot
      end if
      write (iw, &
