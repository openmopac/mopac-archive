...
!            ibonds(j,i) = atom numbers of atoms attached to atom i (there are nbonds(i) of these)
    use molkst_C, only : numat, l1u, l2u, l3u, id
    use common_arrays_C, only : nat, coord, tvec, nbonds, ibonds
    use parameters_C, only : covalent_atom_radius
    implicit none
!-----------------------------------------------
!   L o c a l   V a r i a b l e s
!-----------------------------------------------
    integer :: i, ik, j, jk, kl
   ...
   !
    do i = 1, numat
      ...
      do j = 1, i - 1
        ...
        if (rmin < (1.1d0*(covalent_atom_radius(nat(i))+covalent_atom_radius(nat(j))))**2) then
