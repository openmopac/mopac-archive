...
!
      ...
      moperr, nvar, id, line, last, mozyme, in_house_only
!
      use chanel_C, only: iw, iw0, log, ilog
!
...
!
      ...
      integer :: i, itry1, jcyc, icyc, niwa, nwa, maxcyc, alloc_stat, m = 12, isave(44), &
        nflush = 1
!
   ...
   !       inquire (file=restart_fn, exist = restrt)
      ...
      do
       ...
       !     ------- THE BEGINNING OF THE LOOP ----------
        if (tleft < 1.5d0*cycmx .or. jcyc-icyc+1 > maxcyc) then
          if (jcyc-icyc+1 > maxcyc) then
            write (iw, "(/20x,a,/)") &
       ...
       & 0.d0, wa, iwa, task,-1, csave, lsave, isave, dsave)
        ...
        if (task(1:2) == "FG") then
          if (jcyc > 1) then
            ...
            sum = Sqrt (sum)
            i = min(nvar,10)
            stepmx = Min (1.d0, Sqrt (dot(oldstp, oldstp, i)/i))
...
!  If current point is not the best, then load in the best point
!
  99  if (best_funct < escf .and. Index (keywrd, " LET") == 0 ) then
        escf   = best_funct
        gnorm  = best_gnorm
        xparam = best_xparam
        grad   = best_gradients
...
!
   ...
   & dsave, jcyc, escf)
      ...
      if (mode == 1) then
       ...
       !
        write (ires) wa, iwa, task, csave, lsave, isave, dsave, jcyc, &
       & escf, nscf, tt0
        close (ires)
      else
...
10010     format (/, a4, 1 p, 6(1 x, d11.4), /, (4 x, 1 p, 6(1 x, d11.4)))
      ...
      if (iprint >= 1) then
10020   format (2(1 x, i4), 2(1 x, i5), 2 x, a3, 1 x, i4, 1 p, 2(2 x, d7.1), &
       & 1 p, 2(1 x, d10.3))
...
10140   format (2(1 x, i4), 2(1 x, i5), 2 x, a3, 1 x, i4, 1 p, 2(2 x, d7.1), &
       & 6 x, "-", 10 x, "-")
...
10000   format (/, "----------------SUBSM entered-----------------", /)
    ...
    !     Backtrack to the feasible region.
      ...
      if (iprint >= 99) then
        if (alpha < one) then
10010     format ("ALPHA = ", f7.5, " backtrack to the BOX")
