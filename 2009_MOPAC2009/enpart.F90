...
!-----------------------------------------------
      ...
      & coord, w
      use molkst_C, only : numat, uhf, keywrd
...
!
   ...
   60   continue 
      ...
      write (iw, '(3/,10X,''TOTAL ENERGY PARTITIONING'')')  
...
!
      if (index(keywrd,"ENPART(") /= 0) then
        ...
        keywrd(j:j) = ")"
        if (k <= numat) then
          write(iw,"(11x,a,i1,a)")" System is to be split into ",nparts, " parts"
          write(iw,"(15x,a)")" Part No. of atoms     Atoms"
          k = 0
          do i = 1, nparts
            write(iw,"(16x,i3,i8,i10,' to',i4)")i, parts(i) - k, k + 1, parts(i)
            k = parts(i)
          end do
          il = 1
          do nparts  = 1, nparts
...
!
            ...
            write(iw,"(a,i1,a,f19.4,' eV')") &
            " Part ", nparts," self - energy (ONE-CENTER(E-E + E-N) + EE):",sum
!
...
!
            ...
            do l = 1, nparts - 1
              ...
              jl = ju + 1
              write(iw,"(a,i1,a,i1,a,10x,f19.4,' eV =',f9.3,' Kcal/mol')") &
              " Part ", nparts, " - part ",l," interaction energy (EE):",sum, sum*fpc(9)
            end do
            il = iu + 1
          end do
        end if
