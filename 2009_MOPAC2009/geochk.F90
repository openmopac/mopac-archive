...
!
      if (txtatm(i)(2:2) /= "+" .and. txtatm(i)(2:2) /= "-") &
      & atom_charge(i) = " "
    end do
   !
            ...
            if (maxtxt > 1) then
              if (jj > 0) then
                ...
                k, i, padding(:l)//txtatm(i)(1:maxtxt)//padding(:l), elemnt(nat(i)),ions(i), &
                   "    (",r_ions(1),trim(line)//elemnt(nat(near_ions(1)))//":  "//txtatm(near_ions(1))(:maxtxt)//")"
                if (jj > 1) then
                  do ii = 2, jj
                  write(iw,"(66x, a, f4.1,a)")  "   (",r_ions(ii), &
                    trim(line)//elemnt(nat(near_ions(ii)))//":  "//txtatm(near_ions(ii))(:maxtxt)//")"
                  end do
                end if
              else
