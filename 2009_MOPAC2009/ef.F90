...
! 
   ...
   !
    ih = (nvar*(nvar+1)) / 2
    if (nvar > 7996) then
      write(iw,'(4x,a)') "Insufficient memory to run EF"
      write(iw,'(4x,a)') " ", &
      ...
      if (tleft < (tstep - (t_hess2 - t_hess1))*two) go to 1030
        if ( .not. saddle .and. tstep > 0.5d0 .or. first_time) then
          ...
          if (ldump == 0) then
           ...
           & a1, "  GRAD.:", f10.3, " HEAT:", g13.7)', err=1000) &
           tprt, txt, Min (gnorm,999999.999d0), funct
            end if
            if (nflush /= 0) then
              if (Mod(nstep+1, nflush) == 0) then
...
1000  ihess = ihess + 1
      ...
      rmx = Sqrt (dot(grad, grad, nvar))
      if (rmx < tol2) then
10020   format (/, 5 x, "RMS GRADIENT =", f9.5, "  IS LESS THAN CUTOFF =", &
             & f9.5,//)
         !
         !     ****** OPTIMIZATION TERMINATION ******
         !
        write (iw, 10020) rmx, tol2
        go to 1020
      else
        if (ef_mode == 0 .and. nstep > 5) then
        ...
        olde = funct
        if (nstep > 5) then
          cosine = dot(grad, oldf, nvar)/sqrt(dot(grad, grad, nvar)*dot(oldf, oldf, nvar))
  ...
  !        write(iw,*)"COSINE:", cosine, cos_const
        ...
        call rsp (hessc, nvar, nvar,eigval, u)
...
10030     format (/, 10 x, "HESSIAN HAS", i3, " NEGATIVE EIGENVALUE(S)", &
        ...
        do i = 1, nvar
          fx(i) = dot (u(1 + (i-1)*nvar), grad, nvar)
...
10070     format (5 x, "STEPSIZE USED IS", f9.5)
        ...
        if (iprnt >= 1) then
10080     format (5 x, "NEW TRUST RADIUS = ", f7.5)
          write (iw, 10080) dmax
        end if
...
10090 format (/, 5 x, "TRUST RADIUS NOW LESS THAN ", f7.5, &
...
1030 if (tleft < tstep*two) then
10100 format (/, 5 x, "NOT ENOUGH TIME FOR ANOTHER CYCLE")
      write (iw, 10100)
    end if
    if (nstep >= mxstep) then
10110 format (/, 5 x, "EXCESS NUMBER OF OPTIMIZATION CYCLES")
      write (iw, 10110)
    end if
    ipow(1) = ihess
    ipow(9) = 1
...
subroutine efsav (tt0, hess, funct, grad, xparam, pmat, il, bmat, ipow, &
  ...
  USE maps_C, only : rxn_coord1, rxn_coord2, ijlp, ione, jlp, jlp1, ilp, surf, &
  rxn_coord, latom, kloop
  use molkst_C, only : nvar, keywrd, nscf, is_PARAM, norbs, numat, mozyme
...
subroutine efstr (xparam, funct, ihess, ntime, iloop, igthes, mxstep, ireclc, &
    ...
    use molkst_C, only: limscf, time0, numcal, keywrd, moperr, id, in_house_only 
   ...
   !     GET ALL INITIALIZATION DATA
    ...
    if (in_house_only) then
      if (.not. restrt) then
   !     inquire (file=restart_fn, exist = restrt)
          if (restrt) write(iw,*) &
          " RESTART not specified, but RESTART file found, so job will be RESTARTED"
      end if
    end if
...
10000     format (5 x, "WARNING! MODE SWITCHING. WAS FOLLOWING MODE ", &
      ...
      if (eigit == zero) then
        if (nvar > 3*numat - 5) then
          write(iw,"(5x,a)")" Too many variables. By definition, at least one force constant is exactly zero" 
          write(iw,"(5x,a)")" and the lowest force constant is not negative."
          write(iw,"(5x,a,i5)")" Number of variables = ", nvar
          write(iw,"(5x,a,i5)")" Number of atoms     = ", numat
          if (nvar == 3*numat) then
            write(iw,"(5x,a,i5)")" (If coordinates are Cartesian, convert to internal coordinates and re-run.)"
            call mopend (" Too many variables. (If coordinates are Cartesian, convert to internal coordinates and re-run.)" )
          else
    ...
    do i = 1, nvar
      ...
      if (i == it) then
        if (Abs (lamda0-eigval(it)) < 1.d-9) then
          ...
          write (iw, "(/10X,'CURRENT VALUE OF geo',/)")
          call geout (iw)
...
subroutine gethes (xparam, igthes, iloop, hess, pmat, bmat, grad, geo, loc, &
   ...
   !.. Local Scalars ..
    ...
    double precision :: dg1, dg2, dg3, dummy, fdmy, funct, funct1, sum, &
         & tdm, time1, time2, tstep, tstore, tt0, test=0.d0
    double precision, parameter :: dghsa = 500.d0, dghsd = 200.d0, &
         & dghss = 1000.d0, two = 2.d0, xinc = 1.d-3, zzero = 0.d0
    logical :: lpacifier
   !
   !.. Local Arrays ..
