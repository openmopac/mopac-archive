...
!
   ...
   60 continue 
      ...
      if (gui) then
        i = index(keywrd,"PM5")  ! Convert PM5 to PM6 for CAChe only
        if (i /= 0) then
          keywrd(i:i+2) = "PM6"
           write (iw, '(A)') ' Keyword PM5 was supplied. PM5 is not supported, so keyword converted to PM6' 
        end if
      end if
      return  
  100 continue 
