...
!
      ...
      site_no, method_pm7,  sz, ss2, &
      in_house_only, no_pKa, nalpha_open, nbeta_open, dh
!
      use MOZYME_C, only : icocc, icvir, ncocc, ncvir, nvirtual, noccupied, &
      & nnce, nncf, cocc, cvir, ncf, nce,  cocc_dim, &
      & cvir_dim, icocc_dim, icvir_dim, iorbs, nres
!
...
!
      use chanel_C, only : iw, iarc, ibrz, iw0, brillouin_fn, archive_fn
!-----------------------------------------------
      use meci_I
      use geoutg_I 
      use dipole_I
...
!          CAN BE USED BY THE PATH OPTION)
      ...
      write (iw, "(2/30X,A,' CALCULATION')") trim(caltyp)
      write (iw, '(55X,''MOPAC2009 (Version: '',a,a)') verson, ")"

      write (iw, '(55X,A24)') idate
      if (ijulian < 500) write (iw, '(55x,a,i3)')"No. of days left = ",ijulian
      if (iscf == 2) then 
!
...
!
      ...
      !
        if( id == 1) then
          escf = escf + pressure * Sqrt(dot(tvec(1,1), tvec(1,1),3))
        else if (id == 3) then
          escf = escf + pressure * volume(tvec,3)
        end if
      end if
      if (stress > 1.d-5) then
        write (iw, &
  ...
  !        if (Abs(E_disp) >1.d-10) &
  !          write(iw,'(/,a,F17.5,a)')"     'DH' Dispersion contribution =", E_disp," Kcal/mol"
  !        if (Abs(E_hb) >1.d-10) then
  !          write(iw,'(a,F17.5,a)')  "     'DH' H-bond contribution     =", E_hb," Kcal/mol"
  !        end if
  !      end if
      end if
      ...
      call to_screen(line)
      write (line, '(10X,''GRADIENT NORM           ='',F17.5)') gnorm 
      call to_screen(line) 
      ...
      if (norbs > 0 .and. .not. mozyme) call symtrz (c, eigs, 1, .not. mozyme) 
      if (moperr) return  
      ...
      if (Index(keywrd, " EPS") == 0 .and. id == 0 .and. .not. mozyme) then
        sum = 78.4d0
        fepsi = (sum-1.d0) / (sum+0.5d0)
        call cosini         
        if (moperr) then
!
! An error occurred in COSINI or COSCAV.  Almost certainly caused
! by a failure to allocate memory.
! Ignore the failure, and continue.
!
          endfile (iw)  
          backspace (iw) 
          backspace (iw)    !  Erase the message of the failure.
          backspace (iw)    !  Erase the message of the failure.
          backspace (iw)    !  Erase the message of the failure.
          moperr = .false.
          area = 0.d0
      ...
      if (lprtgra .or. gnorm > 1.D-15 ) write (iw, '(10X,''GRADIENT NORM           ='',F17.5)') &
        gnorm 
...
!
            sum = sqrt(dot(dxyz,dxyz,3*numat))
            if (sum > max(5.D0,5*gnorm**2) .and. gnorm<2.D0.and. &
...
!   CORRECTION TO I.P. OF DOUBLETS
      ...
      if (mol_weight > 0.1D0) write (iw, &
        '(    10X,''MOLECULAR WEIGHT        ='',F15.3)') mol_weight 
      ...
      call gmetry (geo, coord) 
      if (lprtgra.or. gnorm > 2.d0 ) then 
        write (iw, '(3/7X,''FINAL  POINT  AND  DERIVATIVES'',/)')        
        if (mozyme) then
          call prtgra ()
          if (nres /= 0) call modgra ()          
        else
          write (iw, &
...
!
      ...
      '(4X,''NO.'',7X,''ATOM'',15X,''X'',9X,''Y'',9X,''Z'',/)') 
        do i = 1, numat
          if (l_atom(i)) write (iw, '(I6,8X,A2,14X,3F10.4)') i,elemnt(nat(i)),(coord(j,i),j=1,3)
        end do
      else 
      ...
      if (norbs > 0) then 
        ...
        if (.not. in_house_only .and. mers(1) /= 0 .and. .not. mozyme) then           
        ...
        if (nelecs /= 0) then 
          if (index(keywrd,' DENS') /= 0) then 
            ...
            if (mozyme) then
              ...
              write (iw, '(8F10.5)') (pdiag(i), i = 1, norbs) 
            else
        ...
        if (uhf) then 
          sz = abs(nalpha + (nalpha_open - nalpha)*fract - (nbeta + (nbeta_open - nbeta)*fract) )*0.5D0 
          ss2 = sz*sz 
          l = 0 
          do i = 1, norbs 
            do j = 1, i 
...
!
      ...
      if (icalcn /= numcal) then 
        ...
        if ( index(keywrd," PDBOUT") /= 0) then
          open(unit=31, file=archive_fn(:len_trim(archive_fn) - 3)//"pdb", status='UNKNOWN', position='asis') 
          call pdbout(31)
          close (31)
        end if
        icalcn = numcal 
      ...
      if (j /= 0) then
            write(iw,*) "Could not write to archive file, run continuing."
            return
          end if
      write (iwrite, '(55X,''MOPAC2009 (Version: '',a,a)') verson, ")"
      write (iwrite, '(55X,A24)') idate
      if (ijulian < 500) write (iwrite, '(55x,a,i3)')"No. of days left = ",ijulian
      write(iwrite,"(/,a,/)")formula(:len_trim(formula))  
      call wrttxt (iwrite) 
      write (iwrite, '(2/4X,A58)') flepo(iflepo) 
      write (iwrite, '(4X,A58)') iter(iscf)
      if (stress > -0.5d0) then
        write (iwrite, &
    ...
    '(10X,''AVERAGE DISTORTION      ='',F17.5,'' Angstroms per atom (all atoms)'' )') distortion
      ...
      '(    10X,''FOR REACTION COORDINATE =  '',F16.4,'' ANGSTROMS'')')&
        ...
        write (iwrite, '(    10X,''REACTION GRADIENT       ='',F16.6,A14     )'&
          ) gcoord(1,1), grtype 
      endif 
      if(id == 0) then
      ...
      if (uhf) then 
        ...
        write (iwrite, '(  10X,''NO. OF ALPHA ELECTRONS  =  '',I9)') nalpha 
        write (iwrite, '(  10X,''NO. OF BETA  ELECTRONS  =  '',I9)') nbeta 
      else 
      ...
      write (iwrite, '(  10X,''MOLECULAR WEIGHT        =  '',F13.3)') mol_weight 
      if (id == 3) then
      ...
      call dimens (coord, iwrite) 
      write (iwrite, &
        '(  10X,''SCF CALCULATIONS        =  '',I9)') nscf 
      tim = second(1) - time0 
      ...
      subroutine empiri ()
      use molkst_C, only : line, numat, formula
...
!
    ...
    write (iwc, "(    10X,'TOTAL COSMO CHARGE      =',F17.5,/)") &
       & sum
    write (iwc, "(10X,A)") "ATOMIC DATA "
    write (iwc, "(a10,a24,a13,6A12)") " NR. ELEM.", "COORDINATES", &
...
!----------------------------------------------- 
      ...
      do i = 1, 98
        ...
        do j = 1, i
          if (.not.els(j)) cycle        
          write(iwrt,"(12x,a,a2,7x,a,f16.6)") "ALPB_",elemnt(j), elemnt(i), alpb(i,j)
          write(iwrt,"(12x,a,a2,7x,a,f16.6)") "XFAC_",elemnt(j), elemnt(i), xfac(i,j)
        end do
      end do
