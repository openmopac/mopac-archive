...
!
      ...
      method_pm6_dh_plus, method_pm7, method_pm6_plus, in_house_only, jobnam
!
...
!-----------------------------------------------
      ...
      logical :: int = .true., aigeo, xyz, opend, exists, is_PDB
      logical, allocatable :: same(:)
      character :: space, space2*2, ch, ch2*2, idate*24, line_1*80
      save  space, space2, int, ireact
!-----------------------------------------------
!
...
!
        ...
        if (aigeo .or. index(keywrd,' AIGIN')/=0) then 
          ...
          call getgeo (ir, labels, geo, coord, lopt, na, nb, nc, int) 
          if (natoms == -2) then
            rewind(ir)
            call getpdb()
            is_PDB = .true.
...
!    WRITE HEADER
      ...
      write (iw, '(1X,a)')"*******************************************************************************"
      write (iw, '(1X,a)')"** Cite this work as: MOPAC2009, James J. P. Stewart, Stewart Computational  **"
      if (ijulian > 410) then
        write (iw, '(1X,a, a,a)') &
...
!
        ...
        method_rm1 .or. method_mndo .or. method_pm6))   
          ...
          if (method_pm7) then
            write (iw, "(///10X,'UNRECOGNIZED KEY-WORDS: (',A,')')") "PM7"   
            call to_screen('UNRECOGNIZED KEY-WORDS: ')
           call to_screen("PM7")
             call mopend ("KEYWORD PM7 IS RESERVED FOR FUTURE USE.")   
            return 
          end if
        end if
        if ( .not. method_pm6) method_pm6 = (.not.(method_am1 .or. method_pm3 .or. method_mndod .or. &
        method_rm1 .or. method_mndo .or. method_pm7))
      ...
      if (index(keywrd,' SYMM') + index(keywrd," SYM ") /=0 .or. ndep > 0) then 
        ...
        if ((xyz .or. int)) then          
          ...
          if (index(keywrd," IRC") + index(keywrd," DRC") == 0) then           
            ...
            if (int) write(iw,"(13x,a)")"(Remove either SYMMETRY or INT)"
...
!
      ...
      & '' SO THE CALCULATION IS TERMINATED AT THIS POINT'')') 
          ...
          if (index(keywrd,' 1SCF')==0 .or. index(keywrd,' GRAD')/=0) then 
            if (int .and. nvar/=0 .and. nvar<3*numat-6) then 
              write (iw, &
...
!
          ...
          if (.not. exists) then
            write(iw,"(//10x,3a)")"File ", trim(line), " does not exist."
    ...
    99    write(iw,*)" File' "//trim(line)//"' is faulty"
          return
    97    i = 2
          if (refkey(ii)(1:1) /= " ") i = 3
          if (keywrd(j + i:j + i) == " ") then
          ...
          i = natoms       
          call getgeo (99, labels, geoa, c, lopt, na, nb, nc, int) 
          if (moperr) then
            i = index(keywrd," GEO_REF")
