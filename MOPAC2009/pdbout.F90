...
!
    ...
    do i = 1, numat
      ...
      if (letter == "J") then
        if (nores) then
          write (iprt, "(A,F12.3,F8.3,F8.3,A,I5)") "HETATM"//txtatm(i)(22:41), &
          & (coord(k, i), k=1, 3), "  1.00  0.00      PROT", nline
        else
          write (iprt, "(A,I5,A,A3,A,F11.3,F8.3,F8.3,A,I5)") "HETATM", i, blank &
         & (1:j) // ele_pdb (j:2) // "   ", txtatm (i) (9:11), "  " // txtatm (i) &
         & (12:15)//" ", (coord(k, i), k=1, 3), "  1.00  0.00      PROT", nline
       end if
      else
        if (letter == "V") then
          letter = " "
        ...
        if (nores) then
          write (iprt, "(A,F12.3,F8.3,F8.3,A,I5)") "ATOM"//txtatm(i)(20:41), &
        & (coord(k, i), k=1, 3), "  1.00  0.00      PROT", nline
        else                
          if (j == 1) then
            write (iprt, "(A,I5,A,A3,A,F11.3,F8.3,F8.3,A,I5)") "ATOM  ", i, &
