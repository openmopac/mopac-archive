subroutine chkion (ox_calc, n_lone_pairs)
   !***********************************************************************
   !
...
!                              Isolated atom (Most of these will never exist, e.g. C)
    data ox_ref &
        ...
        &  1, 2, 2, 14*3,        4, 3, 2, 3, 2, 3, 2, 1, 2,   3, 4, 3,-2,-1, 0, &!   86
...
!
    data ox_ref_spd1 &
        ...
        &  1, 2, 2, 5*0,3*2,6*2, 4, 5, 6, 7, 2, 3, 2, 1, 2,   3, 4, 3, 4, 0, 0, &!   86
    ...
    do i = 1, mm
      do j = 1, Lewis_tot
        ...
        if (jj < 0) then
          ...
          iz(jj) = iz(jj) - 1
          if (ib(jj) == 1) then
            write (iw,'(/10x,a,i6,a)') " WARNING: Carbon atom ", jj, " has an unexpected valency!"
            if (nbonds(jj) > 0) then
              num = "1"
              do k = 1, nbonds(jj)
