! calculate dC6/dr analytically
      subroutine anagrdc6(max_elem,maxc,n,cn,dcn2,dcn3,iz,c6ab,mxc,iat,jat,kat,anag)
      implicit   none
      double precision :: k3 
      parameter (k3=-4.d0) 
      ...
      do i=1,mxc(iz(iat))
        do j=1,mxc(iz(jat))
          ...
          do k=1,3
            dzaehler(k)=dzaehler(k)+c6ab(iz(iat),iz(jat),i,j,1)*term4*(term2*dterm2(k)+term3*dterm3(k))
...
!
      ...
      double precision :: c6ab(max_elem, max_elem, maxc, maxc, 3), rthr
      double precision :: e6,  e8        
 
      integer :: iat, jat
      double precision :: r, r2, r6, r8, tmp, dx, dy, dz, c6, c8
...
! get C6
            call getc6(maxc, max_elem, c6ab, mxc, iz(iat), iz(jat), cn(iat), cn(jat), c6)
            r6 = r2**3      
            r8 = r6*r2
...
!
      subroutine gdisp(max_elem, maxc, n, xyz, iz, c6ab, mxc, r2r4, r0ab, rcov, &
        s6, s18, rs6, rs8, alp6, alp8, rthr, g, gnorm, rthr2)
      implicit none  
      integer :: n, iz(*), max_elem, maxc, mxc(max_elem)
      double precision :: xyz(3, *), r0ab(max_elem, max_elem), r2r4(*)
      double precision :: c6ab(max_elem, max_elem, maxc, maxc, 3)
      double precision :: g(3, *), s6, s18, rcov(max_elem)
      double precision :: rs6, rs8, alp8, alp6        
 
      integer :: iat, jat, kat
      double precision :: R0, C6, R42, disp, x1, y1, z1, x2, y2, z2, rr  
      double precision :: dx, dy, dz, r2, r, r4, r6, r8, r10, t6, t8
      double precision :: damp6, damp8, gnorm
      double precision :: s8, gC6(3), term
      double precision :: cn(n), rthr, rthr2
      double precision :: dcn2(3, n), dcn3(3, n, n), rthr3
...
! stored as sqrt
            ...
            call getc6(maxc, max_elem, c6ab, mxc, iz(iat), iz(jat),  cn(iat), cn(jat), C6)           
! analytically
            call anagrdc6(max_elem, maxc, n, cn, dcn2, dcn3, iz, c6ab, mxc, iat, jat, iat, gC6)
            r = sqrt(r2)
            t6 = (r/(rs6*R0))**( - alp6)
            ...
            r10 = r4*r6
            dx = 2.D0*x1 - 2.D0*x2
            term = - 3.D0*damp6*damp6*s6*C6/r8*t6*alp6*dx - 1.D0*damp6*s6*gC6(1)/r6 + 3.D0*damp6*s6*C6/r8*dx &
              - 9.D0*damp8*damp8*s8*C6*R42/r10*t8*alp8*dx - 3.D0*damp8*s8*gC6(1)*R42/r8  + 12.D0*damp8*s8*C6*R42/r10*dx
            g(1, iat) = g(1, iat) + term
            dy = 2.D0*y1 - 2.D0*y2       
            term = - 3.D0*damp6*damp6*s6*C6/r8*t6*alp6*dy - 1.D0*damp6*s6*gC6(2)/r6  + 3.D0*damp6*s6*C6/r8*dy &
              - 9.D0*damp8*damp8*s8*C6*R42/r10*t8*alp8*dy - 3.D0*damp8*s8*gC6(2)*R42/r8 + 12.D0*damp8*s8*C6*R42/r10*dy
            g(2, iat) = g(2, iat) + term
            dz = 2.D0*z1 - 2.D0*z2
            term = - 3.D0*damp6*damp6*s6*C6/r8*t6*alp6*dz - 1.D0*damp6*s6*gC6(3)/r6  + 3.D0*damp6*s6*C6/r8*dz &
              - 9.D0*damp8*damp8*s8*C6*R42/r10*t8*alp8*dz - 3.D0*damp8*s8*gC6(3)*R42/r8  + 12.D0*damp8*s8*C6*R42/r10*dz
            g(3, iat) = g(3, iat) + term
            term = - 1.D0/(1.D0 + 6.D0*t6)*s6*C6/r6 - 3.D0/(1.D0 + 6.D0*t8)*s8*C6*R42/r8
            if (iat.lt.jat) then
               disp = disp + term
            endif
          end do       
          do jat = 2, n
              ...
              y1 = xyz(2, jat)
              z1 = xyz(3, jat)
!THR IJ MOST IMPORTANT
              if( (xyz(1, iat) - x1)**2 + (xyz(2, iat) - y1)**2 + (xyz(3, iat) - z1)**2 .gt. rthr2* r0ab(iz(iat), iz(jat))) cycle
              do kat = 1, jat - 1
                if (iat.eq.kat) cycle
                x2 = xyz(1, kat)
                y2 = xyz(2, kat)
                ...
                r2 = dx + dy + dz
!THR IK
                if( (xyz(1, kat) - xyz(1, iat))**2 +  (xyz(2, kat) - xyz(2, iat))**2 + (xyz(3, kat) - xyz(3, iat))**2 &
...
! analytically
                call anagrdc6(max_elem, maxc, n, cn, dcn2, dcn3, iz, c6ab, mxc,  jat, kat, iat, gC6)
                r = sqrt(r2)
                rr = R0/r
                t6 = (rr*rs6)**alp6
                damp6 = 1.d0/( 1.d0 + 6.d0*t6 )
                ...
                r6 = r2*r4
                r8 = r4**2
                term = - 1.D0*damp6*s6*gC6(1)/r6 - 3.D0*damp8*s8*gC6(1)*R42/r8
                g(1, iat) = g(1, iat) + term
                term = - 1.D0*damp6*s6*gC6(2)/r6 - 3.D0*damp8*s8*gC6(2)*R42/r8
                g(2, iat) = g(2, iat) + term
                term = - 1.D0*damp6*s6*gC6(3)/r6 - 3.D0*damp8*s8*gC6(3)*R42/r8
                g(3, iat) = g(3, iat) + term
          end do
        end do
      end do
...
! when nci or ncj is much larger than cn1/cn2
      double precision ::    cn1, cn2, r, rsum, csum,  tmp1,  k3
      parameter (k3=-4.d0) 
...
! ncoord derivative
    subroutine ncoorda(natoms, rcov, iz, xyz, cn, dcn2, dcn3)
      ...
      parameter (k1 = 16.d0)
      integer :: iz(*), natoms, i
      double precision :: xyz(3, *), cn(*), dcn2(3, natoms), rcov(94)
      double precision :: dcn3(3, natoms, natoms)
      integer :: iat
      double precision :: dx, dy, dz, r, xn, rr, rrr, rco, tmp1, tmp2, tmp3
      dcn2 = 0.0d0
      dcn3 = 0.0d0
      do i = 1, natoms
        xn = 0.0d0
