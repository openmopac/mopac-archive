...
C                     THE LENGTH AGREES WITH 'DERI1' FOR COHERENCY.
      COMMON /SCRCHR/ BABINV(3600),BCOEF(6*MAXPAR**2+1-3600)            DL0397
...
C
  200 NBSZE=NBSIZE
     ...
     .   ,F12.9)')NLAST-NBSZE,THROLD,NVAR,TEST2
         ...
         WRITE(JOUT,'('' ELAPSED TIME IN RELAXATION'',F15.3,'' SECOND'')
     .              ')TIME2-TIME1
      ENDIF
      IF(FAIL) THEN
...
C     THE END .
  390 CONTINUE
      IF(DEBUG) THEN
        ...
        WRITE(JOUT,'('' ELAPSED TIME IN C.I-ENERGY RELAXATION'',F15.3,
     .                '' SECOND'')')TFLY-TIME2
      ENDIF
      RETURN
      END
