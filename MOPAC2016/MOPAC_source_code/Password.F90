  ...
  if (i /= 0) then
    ...
    if (mopac_pw(2:2) ==":" .or. index(mopac_pw, "$") /= 0) then  !  Is a Windows system
      ...
      if (exists) then
        ...
        write(0,*)" Environmental variable ""MOPAC_LICENSE"" exists, but its value"
        write(0,*)" does not represent a folder on this computer that contains MOPAC2016.exe"
        write(0,"(a)")"(An attempt was made to detect the file '"//trim(mopac_pw)//"MOPAC2016.exe')"
        stop
      end if
      ...
      if (exists) then
        ...
        write(0,*)" but the directory does not contain the MOPAC2016 executable"
...
!  
      ...
      if (exists) then
        ...
        if ( .not. exists) then
          write(0,*) 
          write(0,*) '  The MOPAC2016.exe file MUST be put in folder C:\program files\mopac'
...
!
        write(0,'(a)')' The MOPAC executable must be put into a directory named "/opt/mopac"'
        write(0,'(a)')' (If it cannot be put there, use environmental variable MOPAC_LICENSE to re-define the directory)'
 ...
 98 continue
  ...
  if (.not. exists) then
!
! Password for MOPAC2016 does not exist, so check for Password for MOPAC2009
!
    ...
    if (exists) then
      p_new = 76
      open(p_new, file = mopac_pw(:i)//"16", form="unformatted")
    else
...
!
    ...
    !  i = from_MAC_address()
      ...
      write(0,'(10x,a)')" 'MOPAC2016.exe <license-key>'"
      write(0,'(10x,a)')" e.g., 'MOPAC2016.exe 1234567a1234567'"
...
!
    ...
    if ( .not. site_ok) then
      write(0,"(//,a)")" Before MOPAC2016 can be used, a password must be supplied"
      ...
      write(0,"(a)")" For example, if the password is 1234567a12345678, then run ""MOPAC2016.exe 1234567a12345678"""
      close(7, status="delete")
    ...
    open(unit=7, file=trim(mopac_pw), form="unformatted", iostat=i)
    if (i == 9) then
      write(0,'(//10x,a)')" The file "//trim(mopac_pw)//" cannot be created - check permissions"
      if (verson(7:7) == "W") &
        write(0,'(/10x,a,//)')" For instructions, see: http://openmopac.net/Windows_set_permissions.html"  
      call sleep(20)        
    end if
...
!
 ...
 " Press (return) to continue."
  ...
  if (.not. site_ok) then
    ...
    write(0,"(a)")" "
    write(0,"(a)")" To install the password, run MOPAC with the file name equal to the password"
    write(0,"(a)")" For example, if the password is 123456789, then run ""MOPAC2016.exe 123456789"""
    write(0,"(//,a)") "Password invalid"
    open(unit=7, file=trim(mopac_pw), form="unformatted")
    close(7, status="delete")
    stop
  ...
  if (new) then
    write(0,"(//a,////)")"         Password for MOPAC2016 successfully installed. Enjoy!"
    stop
  end if 
