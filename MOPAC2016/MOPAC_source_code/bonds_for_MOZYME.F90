      ...
      if (nat(i) /= 1 .or. lall) then
        ...
        if (mozyme_style) then         
          if (l /= 0) then
            ...
            if (maxtxt /= 0) then
              write (iw, "(I6,1X,A,A,F5.3,A1,a,F6.3)") i, elemnt (nat(i)) &
             & //"("// txtatm (i)//")", "     (", valenc, ")", &
             & elemnt(nat(ibab(1)))//"("//txtatm(ibab(1))//")", bab(1) 
              do j = 2, ju
                write(iw,'('//nc//'x, a, f6.3)') elemnt(nat(ibab(j)))//"("//txtatm(ibab(j))//")", bab(j)
              end do
            else
              write (iw, "(I6,1X,A,A,F6.3,')',6(I6,1x,A2,F6.3))") i, elemnt(nat(i)), &
