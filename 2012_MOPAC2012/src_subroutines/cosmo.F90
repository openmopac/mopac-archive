...
subroutine coscav
  ...
  !  Check lipa size
   ...
   ! NOW THE CLOSURE OF THE CONCAVE REGIONS OF THE SURFACE WILL BE DONE
    if (ioldcv == 0) then
      call surclo (coord, nipa, lipa, din, rsc, isort, ipsrs, nipsrs, nat, &
...
subroutine extvdw (vdw, refvdw)
    ...
    double precision, external :: wrdkey
   ...
   i = index(keywrd," VDW(")
    ...
    do i = 1, 107
      ...
      vdw(i) = wrdkey (keywrd, "VDW(", 4, ";"//elemnt(i) (:j), j+1, refvdw(i))
    end do
    !
    ...
    do i = 1, numat
      ...
      if (vdw(nat(i)) > 900.d0) then
        write (iw,*) "MISSING VAN DER WAALS RADIUS " // elemnt (nat(i))
        j = 2
        if (elemnt(nat(i)) (2:2) == " ") j = 1
        write (iw, "(2x,3a)") "To correct this, add keyword 'VDW(:", &
       & elemnt (nat(i)) (1:j), "=n.nn)'"
        call mopend ('MISSING VAN DER WAALS RADIUS'//elemnt(nat(i)))
        return
      end if
    end do
