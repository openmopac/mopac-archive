...
!-----------------------------------------------
      integer :: i, l, j, iskin,   iloop, k, kl, ncoprt, bigcycles, &
        ii, i1, maxcyc, iupper, ilp, io_stat, ilim, iw00, percent
...
!
        write (iw, '(10x, A)') ' INITIAL VELOCITY IN DRC' 
...
!   startv = startv*0.02d0
        if (addonk > 1.d-4) startv = startv*addonk      
      else 
!
! undamped DRC
!
...
!
        ...
        if (debug) then
          ...
          write(iw,"(a,  f9.4)")" 'Time' interval (fs):",deltat*1.d15
...
!
        ...
        if (abs(half) < 0.001d0) then 
          ...
          etold = escf + elost1 
    !      write(iw,'(3F15.5)')average_old_hof, average_new_hof, average_old_hof - average_new_hof
