subroutine getpar
  ...
  !**********************************************************************
    ...
    numvar, ifiles_8, locked, penalty, contrl, &
    maxpms, locvar
    use chanel_C, only: iw, ir
    use parameters_C, only : udd, zd, &
    f0sd, g2sd, main_group, f0sd_store, &
    g2sd_store, dorbs, partyp, defmin, defmax 
    implicit none
    character :: typtxt
    character (len=240) :: text, text2
    character (len=120) :: koment
    
    character (len=2) :: elemnt2
    logical :: lfirst = .true., buggy = .false., use_list, par
    integer :: i, j, k, l, jelmnt, &
...
!
    ...
    !    write(text,"('FN',i2,' XX ',a)") j, trim(text2(i + 1:))
      end if
      use_list = .false.
!
...
!
      ...
      do iele = 1,nele  ! iele loop
        ...
        mpar_loop: do ipar = 1,mpar
          ...
          do j = 1, 37
            if (Index(text, partyp(j)) /= 0) go to 1500
          end do
          if (text == " ") then
            numvar = numvar - 1
            cycle
...
!
        ...
        !  sum = valvar(numvar) - Nint(valvar(numvar))
        !  multiplier = 1.d7
        !   if (.not. force_opt) then
        !     i = Nint(sum*multiplier) !  Isolate digits after the seventh decimal -
        !      sum = (sum*multiplier - i)/multiplier    !  so parameters defined "by hand" will not be changed
        !   end if
        !   if (Abs(sum) < 0.1d0/multiplier .and. lall) then
        !     numvar = numvar -1
        !     cycle
        !   end if
...
!
          ...
          if (penalty_fn > 1.d-6) then
            if (i == 99 .or. i == 98) then
              write (ifiles_8, "(12X,A,11X,F16.8,A1,2F16.2)") &
              & "PAR"//text(:2),  valvar(numvar), typtxt, &
    ...
    write (ifiles_8, "(' ELEMENT NOT FOUND (Debug this - how did it get here?')")
    stop
