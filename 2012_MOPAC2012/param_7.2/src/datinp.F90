...
!
    ...
    method_pm7, method_PM6_DH_plus, nalpha_open, nbeta_open
!
...
!
    use chanel_C, only : iw, ir, igpt, ilog
!----------------------------------------------------------------------------
    use second_I
    use reada_I
    implicit none
   ...
   & itime, j, k, l, loop, lref, misd, misg, mish, misi, &
   & mmmols, mol, ndips, nel, nions, nmolmx, nnmols, notok, icapz, &
   & m, lnmols, is, iref, faulty, max_atom, ni, nj, ne_excluded = 0, &
...
!
  ...
  !
    ...
    do i = 1, numvar
      elemok(1, mod(locvar(2, i),200)) = .true.
    end do
  ...
  !
    contrl = keywrd(1:241)
    open (unit=56, file=Trim (jobnam) // ".usd", status="UNKNOWN")
    open (unit=55, file="HOFS.TXT", status="UNKNOWN", iostat=i)
    if (i /= 0) then
     open (unit=55, file="HOFS1.TXT", status="UNKNOWN", iostat=i)
    end if
    if (i /= 0) then
     open (unit=55, file="HOFS2.TXT", status="UNKNOWN", iostat=i)
    end if
    i = 0
    m = 0
    do
      ...
      if ( k /= 0) then
        ...
        moperr = .false.
        close(ir)
        j = min(Index(contrl(k+5:),";"),Index(contrl(k+5:)," ")) + k + 5
        j = Index(contrl(k+5:j),";") + k + 3
        if (j == k + 3) j = Index(contrl(k+5:)," ") + k + 3
        open (unit=ir, file=contrl(k+5:j), status="OLD", err=99)
        goto 89
  99    write(*,"(///,3a,///)")" Reference data file set: '",contrl(k+5:j),"' missing."
        write(ifiles_8,"(///,3a,///)")" Reference data file set: '",contrl(k+5:j),"' missing."
        
        call finish
        stop
  89    continue
        write(ifiles_8,"(30x,3a)")" File set: '",contrl(k+5:j),"' used."
        j = k + 5 + index(contrl(k+5:)," ")
        if (Index(contrl(k:j), ";") == 0) then
          contrl(k:j - 1) = " "  ! Erase "SET=<text>
        else
          j = k + 5 + index(contrl(k+5:j),";")
          contrl = contrl(1:k + 4)//contrl(j:) ! Erase current set 
        end if
      else
        if (i /= 0) exit  !  At least one set of data have been read in.
      end if
...
1100    do k = 80, j, -1
        ...
        do j = 1, i - 1
          if (names(i) == names(j)) then
            write(ifiles_8,"(5x,3a)")" File: """,names(i)(1:len_trim(names(i))),""" already specified."
            i = i - 1
            exit
          end if
        end do
      ...
      cycle
 1200 continue
      if (m == 1) stop !  no data on file-names
    end do
  !
  !  Close input data set to prevent it being damaged by next set
  !  of READS
  !
     if (M == 0) close (ir, status="DELETE")
     if (M /= 0) close (ir, status="KEEP")
...
!
      do l = 1,10
        ...
        if (i /= 0) then
          nref = nref + 1
          refdir(nref) = contrl(k:k+i-2)
          k = k + i 
        end if
        if (i == 0) exit
      end do
      nref = nref + 1
      refdir(nref) = contrl(k:j-1)
      else
      nref = nref + 1
      refdir(nref) = "./"
     end if
     do i = 1,nref
    lref = Index (refdir(i), " ") - 1
    if (lref /= 0) then
      if(refdir(i)(lref:lref) /= "/") then
        lref = lref + 1
        refdir(i)(lref:lref) = "/"
      end if
    end if
...
!
        do loop = 1, 2
          ...
          if (loop == 2) then
          !
          !  There is an error in an input reference data set.
          !  Open an output file to write out diagnostic messages
          !
            i = Index (jobnam, " ") - 1
            inquire (file=jobnam(:i)//".err", exist=exists)
            if ( .not. exists) then
              open (unit=iw, file=jobnam(:i)//".err", status="NEW")
            end if
            write (iw,*) " Diagnostic output for " // refnam (1:30)
          end if
        !
        ...
        !
          ...
          do iref = 1, nref
            refnam = refdir(iref)(1:len_trim(refdir(iref))) // &
            & names (nmols) (:len_trim(names(nmols))) 
            do i = 1, len_trim(refnam)
              iline = Ichar (refnam (i:i))
              if (iline >= icapa .and. iline <= icapz) refnam (i:i) = Char(iline+ilowa-icapa)
            end do
...
1300      line = " "
          ...
          rewind (ir)
          if (l /= 0) then
            write (ifiles_8,*) refnam (1:len_trim(refnam)), "  ++++ NOT FOUND ++++ %"
            nnmols = nnmols + 1
          else
            rewind (2)
          !
...
!
          ...
          j = j + i
          if (i /= 0)  keywrd (i:Index (keywrd(i+1:)," ") + i) = " "
          if (j == 0) exit
        end do
...
!
          ...
          if (method_PM6_DH2)     keywrd(i:i+7) = "PM6-DH2X"
          if (method_pm7)    then
            if (index(keywrd,"PM7") == 0) then
         !       keywrd(i:i+6) = "PM7-DH2" 
           !   else
                keywrd(i:i+4) = "PM7 "
              end if
          end if
          i = Index (keywrd, " PRECISE")
          if (i /= 0) keywrd (i:i + 7) = " "
            if(.not. large) then
            i = Index (keywrd, " HCORE")
            if (i /= 0) keywrd (i:i + 5) = " "
            i = Index (keywrd, " FOCK")
            if (i /= 0) keywrd (i:i + 4) = " "
            i = Index (keywrd, " PL")
            ...
            if (i /= 0) keywrd (i:i + 7) = " "
            i = Index (keywrd, " COMPFG")
            if (i /= 0) keywrd (i:i + 7) = " "
            i = Index (keywrd, " OLDENS")
            if (i /= 0) keywrd (i:i + 7) = " "
            i = Index (keywrd, " NOLOG")
            if (i /= 0) keywrd (i:i + 6) = " "
            ...
            if (cp) then
              i = Index(keywrd,"                   ")
              if (i /= 0) then
                keywrd(i:i+18) = " THERMO(298,298,1) "
              end if
            end if
          end if
!***********************************************************************
...
!
         if( precise) then
       !  i = Index(keywrd,"              ") ! 14 spaces
      !   keywrd(i:i+13) = " RELSCF=1.d-3 "
         i = Index(keywrd,"         ") ! 9 spaces
         keywrd(i:i+8) = " PRECISE "
         end if
     !     i = Index(keywrd,"          ") ! 9 spaces
     !    keywrd(i:i+9) =   " EPS=78.4 "
          if (natoms == 0 .or. moperr) then
            write (ifiles_8,*) refnam (1:30), "  ++++ FILE FAULTY ++++ %"
            write (ifiles_8, "(3A)") " (DATINP) There is an error in data-&
           &set ", names (nmols)
            if (moperr) then
            ...
            if (let .or. core_core_OK(geo, nat, numat, k, l, ccp) .or. .not. method_pm6) then 
             ! write(56,"(a)")refnam(i + 1:len_trim(refnam)-4)
              if (loop == 1) write(56,"(a)")trim(names(nmols))
            else
              write(ifiles_8,"(//,3a,//)")"* "//refnam(i + 1:len_trim(refnam)-4), "  Missing:  ", elemnt(k)//" - "//elemnt(l)
              go to 1400
            end if
...
!
                ...
                write (ifiles_8, "(3A)") " (DATINP) There is an error in data-set ", &
                names (nmols)
                refok = .false.
                mmmols = mmmols + 1
                cycle
...
!
                if (ihrefs(1,k) > 0) then
                  faulty = faulty + 2
                  deadly = .true.
                  write (ifiles_8,"(/,2a)") "  ++++ FILE FAULTY ++++ %: ", refnam (1:len_trim(refnam))
              ...
              i = Index (title, " HWT=")
              if (i /= 0) then
                weight(1, nmols) = reada (title, i) * wth
              end if
!
...
!
            ...
            if (i /= 0 .and. Index(title,"IR=PW91D") == 0) then
              ...
              if (title (i+2:i+2) == "=") then
                lions(nmols) = 0
            !  else if (title (i+2:i+2) == "A") then
         !       lions(nmols) = -10000
         !     else if (title (i+2:i+2) == "E") then   WARNING
         !       lions(nmols) = -20000
         !     else if (title (i+2:i+2) == "P") then
          !      lions(nmols) = 0
              else
...
!
            if (heats (nmols) < -999.9950d0) then
             write(55,"(f8.2)",err=95) heats (nmols)
            else if (heats (nmols) < -99.9950d0) then
             write(55,"(f7.2)",err=95) heats (nmols)
             else if (heats (nmols) < -9.9950d0) then
             write(55,"(f6.2)",err=95) heats (nmols)
             else if (heats (nmols) < -0.9950d0) then
             write(55,"(f5.2)",err=95) heats (nmols)
             else if (heats (nmols) < 0.0001d0) then
             write(55,"(f4.2)",err=95) heats (nmols)
             else if (heats (nmols) < 9.9950d0) then
             write(55,"(f4.2)",err=95) heats (nmols)
             else if (heats (nmols) < 99.9950d0) then
             write(55,"(f5.2)",err=95) heats (nmols)
             else if (heats (nmols) < 999.9950d0) then
             write(55,"(f6.2)",err=95) heats (nmols)
            end if
  95        endfile (ifiles_8)
            backspace (ifiles_8)
            if (moperr) then
              write (ifiles_8, "(3A)") " (READMO) There is an error in data-set", &
              names (nmols)
              write (ifiles_8, "(2A)") " Error message:", errtxt (1:50)
              moperr = .false.
              faulty = faulty + 1
         ...
         !     call state ()
              if (moperr) then
                write (ifiles_8, "(3A)") " (STATE)  There is an error in d&
               &ata-set ", names (nmols)
                write (ifiles_8, "(2A)") " Error message:", errtxt (1:50)
                moperr = .false.
                faulty = faulty + 1
                ...
                if (moperr) then
                  ...
                  write (ifiles_8, "(3A)") " (MOLDAT) There is an error in&
                 & data-set ", names (nmols)
                  write (ifiles_8, "(2A)") " Error message:", errtxt (1:50)
                  moperr = .false.
                  faulty = faulty + 1
                  go to 1600
...
!
                  ...
                  if (weight(1,nmols) + weight(2,nmols) + weight(3,nmols) + weight(4,nmols) < 1.d-4) then
                    if (weight(1,nmols) + weight(2,nmols) + weight(3,nmols) + weight(4,nmols) < 1.d-10) then
                      faulty = faulty + 2
                      deadly = .true.
                      write (ifiles_8,"(/,3a,/)") "  ++++ FILE FAULTY ++++ %: ", refnam (1:len_trim(refnam)), &
                        & ': No reference data '
                      end if
                      goto 1400
                    end if                    
...
1600    if (loop == 2) then
    ...
    !
    !  Remove calculation type keyword.
    !
    ! keywrd (it:it+4) = " "
    !
...
1700 continue
 ...
 ! iw = jfiles(1)
  ...
  if (mish+misd+misi+misg /= 0) then
    if (mish /= 0) then
      write (ifiles_8, "(I4,A)") mish, " REFERENCES FOR HEATS OF FORMATION MISSING"
    else
      write (ifiles_8, "(A)") " ALL HEATS OF FORMATION REFERENCED."
    end if
    if (misd /= 0) then
      write (ifiles_8, "(I4,A)") misd, " REFERENCES FOR DIPOLES MISSING"
    else
      write (ifiles_8, "(A)") " ALL DIPOLES REFERENCED."
    end if
    if (misg /= 0) then
      write (ifiles_8, "(I4,A)") misg, " REFERENCES FOR " // "GEOMETRIES MISSING"
    else
      write (ifiles_8, "(A)") " ALL GEOMETRIES REFERENCED."
    end if
    if (misi /= 0) then
      write (ifiles_8, "(I4,A)") misi, " REFERENCES FOR IONIZATION POTENTIALS MISSING"
    else
      write (ifiles_8, "(A)") " ALL IONIZATION POTENTIALS REFERENCED."
    end if
  else
    write (ifiles_8, "(A)") " REFERENCES PRESENT FOR ALL DATA."
  end if
  if (mmmols /= 0) then
...
!
  ...
  do i = 1, numvar
    k = 1
    if(locvar(1,i) /= 34 .and. locvar(1,i) /= 35) then
...
!
  ...
  do i = 1, numvar
    k = 1
    if(locvar(1,i) == 34 .or. locvar(1,i) == 35) then
      nj = locvar(2,i)/200
...
!
 ...
 & .or. nmols > maxmol) then
  ...
  !  write (ifiles_8, "(' MAXPAT:',I8)") maxpat
...
logical function core_core_OK(geo, nat, numat, ni, nj, ccp)
  ...
  integer :: i, j
  double precision, dimension (3,300) :: coord
  if (first) then
    first = .false.
!
