...
!
   ...
   & .false., large, opend, cp, one_scf
    ...
    double precision, dimension (300) :: errors, yparam
...
!------------------------------------------------------------------------
   ...
   &     Dipole         Diff. Ref.')")
    ...
    !
      ...
      if (one_scf) then
        call compfg (xparam, .true., escf, .true., sum, .false.)      
      else
    ...
    !
      if ((Abs (refher) > 1.d-20 .or. natoms == 1 .and. refhof > 0.d0) .and. (index(koment, " TS") /= 0 .or. .not. one_SCF)) then
       ...
       & koment, refhof, refher + refhof, refher, reftxt(loop, 1), els, names &
       & (loop)
   !     if (method_pm6) write(51,"(a)")koment(:len_trim(koment))
    ...
    !
      if (Abs (refcer) > 1.d-20) then
       ...
       & koment, refips, refcer + refips, refcer, reftxt(loop, 2), els, names &
       & (loop)
  !      if (method_pm6) write(52,"(a)")koment(:len_trim(koment))
    ...
    !
      if (Abs (refder) > 1.d-20) then
       ...
       & koment, refdip, refdip + refder, refder, reftxt(loop, 3), els, names &
       & (loop)
