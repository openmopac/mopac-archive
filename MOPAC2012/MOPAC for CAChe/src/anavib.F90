...
!
  ...
  100     continue 
          linear = i 
          call ntmWrtFreq(k, eigs(k), dipt(k))   ! Write frequency and transition dipole
          sum = 1.D0/tot 
          do ij = 1, linear 
            ...
            call ntmWrtFreqAt(k, i, j, e)   ! Write atom contributions
            shift = sqrt(shift) 
      ...
      '(/,'' VIBRATION'',I5,I5,A4,''    ATOM PAIR        ENERGY CONTRIBUTION    RADIAL'')') &
       k, jndex(k), namo(k) 
          ...
          call ntmWrtFreqAt(0, 0, 0, 0.0d0)   ! end of data
        endif 
        if (vib1) then 
