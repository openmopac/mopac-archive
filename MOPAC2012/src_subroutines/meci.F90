...
!-----------------------------------------------
      ...
      & vectci, xy, dijkl, ispin, eig, ispqr,  k, deltap
...
!-----------------------------------------------
      ...
      integer :: icalcn, ndoubl, j, l, mdim, lroot, ne, lima, &
        ...
        ndown, loc, i2, j2, kroot = 0, iroot, i, ii, ji, m, maxvec, &
      ...
      character :: srep(maxci)*15, root_ir*4, tex_root*1
      ...
      save spin, debug, large, lspin1, first1, sing, doub, trip, quar, quin, &
        ...
        oscil, nperma, npermb, nfa, kroot, limci, nupp, ndown, root_ir
!-----------------------------------------------
!**********************************************************************
!
!                 PROGRAM MECI
...
!
      ...
      '(/,'' CONFIGURATIONS CONSIDERED IN C.I.      '',/,'' M.O. NUMBER :      '',20I4)') &
       ...
       if (nmos > 20) write (iw, '(20X,20I4)') (i,i=nelec + 21, nelec + min(40,nmos)) 
        write (iw, '(''          ENERGY'')') 
        do i = 1, lab 
          write (iw, '(/10X,I4,6X,20I4)') i, (microa(k,i),k=1,min(20,nmos)) 
...
!
      ...
      if (last == 3 .and. prnt2 .or. kroot /= 1 .and. first1 .or. root_ir /= "XXXX") then
        labsiz = lab
      end if
...
!
      ...
      jndex(i) = qn_temp(k, m)
      if (kroot < 1 .and. (smult < 0 .or. ispin(i) == smult)) then 
        iroot = iroot + 1 
...
!
  ...
  510     continue 
      ...
      state_Irred_Rep = namo(kroot) 
      state_spin = tspin(ispin(kroot))
      state_QN = jndex(kroot)

      if (prnt) then 
        call ciosci (c(1,nelec+1), kroot, oscil, conf) 
        deltapp = 0.d0
        delta = 0.d0
  ...
  580 format(i5,a,2f12.6,i6,a1,2x,a8,4x,a4,3f8.4) 
      if (kroot == 0) then 
