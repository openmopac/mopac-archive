...
!-----------------------------------------------
      ...
      & keywrd, enuclr, n2elec, efield, mpack
...
!-----------------------------------------------
      ...
      real(double), dimension(2025) :: wjd, wkd 
      real(double), dimension(3) :: xj 
      real(double), dimension(9,9) :: dibits 
      real(double) ::  xf, yf, zf, const, fldcon, hterme, &
        fnuc, half, enuc 
...
!
          ...
          write (iw, &
      '(/10X,''THE ELECTRIC FIELD IS'',3F10.5,'' VOLTS/ANGSTROM'',/)') xf, yf, zf 
        endif 
        const = a0/ev
!
        efield(1) = xf*const 
