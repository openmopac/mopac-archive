subroutine ef (xparam, funct)
   ...
   !*********************************************************************
    character :: txt
    logical :: lorjk, lrjk, lts, geo_ref
...
! 
    ...
    double precision :: ss, absmin, deact, depre, dtmp, &
         ...
         & best_gnorm, cosine, cos_const, sum, rms
   ...
   !
    ...
    if (icalcn /= numcal .or. nvar*nvar /= i) then
      ...
      geo_ref = (Index (keywrd, " GEO_REF") /= 0)
...
1000  ihess = ihess + 1
      ...
      if (geo_ref) then
        ...
        " = total,", sum/i, " = Average,", sqrt(rms/i)," = RMS movement, in Angstroms"            
      end if
      !
