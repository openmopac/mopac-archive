...
!
  use molkst_C, only : numat, keywrd, line, numcal, E_disp, E_hb, &
    method_pm7, method_PM6, nscf, l123, l1u, l2u, l3u, id, temp_1, temp_2, itemp_1
...
!
      ...
      D2X = (index(keywrd, "PM6-D2X") + index(keywrd, "PM6-DH2X") /= 0 ) 
      if (itemp_1 == -99) then
!
!  Use pre-set fragments 
!
        do nsa = 1, numat
          if (set_a(nsa) == 0) exit
        end do
        ...
        if (j /= 0 .or. method_PM7) then
          ...
          if (i /= 0) then
            ...
            i = i + j
            j = index(keywrd(i:l),")") + i - 2
            line = " "//keywrd(i : j) ! line holds atoms list for first set
  !
...
!
! Delete text associated with this range
!
    text(i:i) = "0"
    do k = j + 1, len_text
