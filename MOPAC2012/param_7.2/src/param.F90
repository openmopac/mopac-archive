...
!
  ...
  !
    ...
    keywrd = contrl
    call parkey (contrl)
    method_mndo = (index(keywrd," PM5") /= 0)
    if (method_mndo) method_pm6 = .false.
...
!
      ...
      if (index(contrl, "PARALLEL") /= 0) call parallel
    !
...
!                   (ichar(julian(5:5)) - ichar('0'))) 
 ! write(*,'(10x,a,i4,a)')" This copy of PARAM has", date_created - todays_date ," days left"
  ...
  subroutine parallel
!
!  Update PM7.txt and read parameters
!
  use dfport
!
!
  use molkst_C, only : jobnam, keywrd
!
  use param_global_C, only : contrl, ifiles_8
  implicit none
  integer :: i, ipm7 = 33
  character :: line*120
  logical :: opend
...
!  At this point:
!
!   The partially optimized parameters are in trim(jobnam).rp
!   There are other parameters in ../PM7.txt
!
!
! Add the current parameters to the end of ../PM7.txt
      line = "M:\utility\cat.exe "//trim(jobnam)//".rp >> ../pm7.txt"
!
      i = int(SYSTEM(trim(line)))
      if (i == -555) return ! dummy
!
! Read in the parameters from PM7
!
      keywrd = contrl
      write(ifiles_8,"(//20x,a,//)") " Parameters read in from PM7.txt, etc"
      call datin(ifiles_8)
!
! Re-write PM7.txt
!
      inquire (unit=ipm7, opened=opend)
      if (opend) close (ipm7)
      open(ipm7, file = "..\PM7.txt", iostat = i)
      if (i == 0) then
        call write_params(ipm7)
        close (ipm7)
      end if
