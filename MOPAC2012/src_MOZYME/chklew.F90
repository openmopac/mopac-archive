...
!
!***********************************************************************
    use molkst_C, only: natoms, numat, keywrd, norbs
...
!
    ...
    do i = 1, natoms
      ...
      plus_eight = txtatm(i)(8:8)
      txtatm(i)(8:8) = " "
      if (Index (txtatm(i), "+") /= 0) then
        icharges = icharges + 1
        iz(j) = iz(j) - 1
        ions(j) =  1
      else if (Index (txtatm(i), "-") /= 0) then
        icharges = icharges + 1
        iz(j) = iz(j) + 1
        ions(j) = -1
      end if
      txtatm(i)(8:8) = plus_eight
    end do
!
      ...
      do loop = 1, numat
        ...
        if (ib(ii) < 0 .or. ib(jj) < 0) then
         if (Index (keywrd, " RESEQ")+Index (keywrd, " SITE")+Index (keywrd, " 0SCF") == 0) then
            write(iw,'(a,2i5)')" Impossible user-supplied pi bond: ",ii, jj
            call mopend(" Impossible user-supplied pi bond.")
            return
          end if
        end if
      end do
